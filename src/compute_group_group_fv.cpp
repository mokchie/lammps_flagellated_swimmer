...
ComputeGroupGroupFV::ComputeGroupGroupFV(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  group2(NULL)
{
  if (narg < 4) error->all(FLERR,"Illegal compute group/group/fv command");
  ...
  if (jgroup == -1)
    error->all(FLERR,"Compute group/group/fv group ID does not exist");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"pair") == 0) {
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal compute group/group/fv command");
      if (strcmp(arg[iarg+1],"yes") == 0) pairflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) pairflag = 0;
      else error->all(FLERR,"Illegal compute group/group/fv command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"inertial") == 0) {
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal compute group/group/fv command");
      if (strcmp(arg[iarg+1],"yes") == 0) inertialflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) inertialflag = 0;
      else error->all(FLERR,"Illegal compute group/group/fv command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"velocity") == 0) {
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal compute group/group/fv command");
      if (strcmp(arg[iarg+1],"yes") == 0) velocityflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) velocityflag = 0;
      else error->all(FLERR,"Illegal compute group/group/fv command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"vector") == 0) {
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal compute group/group/fv command");
      if (strcmp(arg[iarg+1],"yes") == 0) vectorflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) vectorflag = 0;
      else error->all(FLERR,"Illegal compute group/group/fv command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"conseronly") == 0) {
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal compute group/group/fv command");
      if (strcmp(arg[iarg+1],"yes") == 0) conseronlyflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) conseronlyflag = 0;
      else error->all(FLERR,"Illegal compute group/group/fv command");
      iarg += 2;
      ...
      if (molflag != OFF && atom->molecule_flag == 0)
        error->all(FLERR,"Compute group/group/fv molecule requires molecule IDs");
      iarg += 2;
    } else error->all(FLERR,"Illegal compute group/group/fv command");
  }
...
ComputeGroupGroupFV::~ComputeGroupGroupFV()
{
  delete [] group2;
  delete [] vector;
}
...
void ComputeGroupGroupFV::init()
{
  ...
  if (pairflag && force->pair == NULL)
    error->all(FLERR,"No pair style defined for compute group/group/fv");
  if (force->pair_match("hybrid",0) == NULL && force->pair->single_enable == 0)
    error->all(FLERR,"Pair style does not support compute group/group/fv");

  // error if Kspace style does not compute group/group/fv interactions

  if (kspaceflag && force->kspace == NULL)
    error->all(FLERR,"No Kspace style defined for compute group/group/fv");
  if (kspaceflag && force->kspace->group_group_enable == 0)
    error->all(FLERR,"Kspace style does not support compute group/group/fv");
  ...
  if (kspaceflag) {
    ...
    if (fabs(e_correction) > SMALL && comm->me == 0) {
      char str[128];
      sprintf(str,"Both groups in compute group/group/fv have a net charge; "
...
void ComputeGroupGroupFV::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
double ComputeGroupGroupFV::compute_scalar()
{
...
void ComputeGroupGroupFV::compute_vector()
{
  invoked_scalar = invoked_vector = update->ntimestep;
...
void ComputeGroupGroupFV::pair_contribution()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        if(!vectorflag)
          ...
          eng = pair->single_vec(i,j,itype,jtype,rsq,factor_coul,factor_lj,conseronlyflag,fpair_vec);
        ...
        if (velocityflag){
          if (newton_pair || j < nlocal) {
            if (ij_flag) {
              if(!vectorflag){
                one[0] += delx*fpair*v[i][0] + dely*fpair*v[i][1] + delz*fpair*v[i][2];
                one[1] += delx*fpair*v[i][0];
                one[2] += dely*fpair*v[i][1];
                one[3] += delz*fpair*v[i][2];
              } else {
                one[0] += fpair_vec[0]*v[i][0] + fpair_vec[1]*v[i][1] + fpair_vec[2]*v[i][2];
                one[1] += fpair_vec[0]*v[i][0];
                one[2] += fpair_vec[1]*v[i][1];
                one[3] += fpair_vec[2]*v[i][2];
              }
            }
            if (ji_flag) {
              if(!vectorflag){
                one[0] -= delx*fpair*v[j][0] + dely*fpair*v[j][1] + delz*fpair*v[j][2];
                one[1] -= delx*fpair*v[j][0];
                one[2] -= dely*fpair*v[j][1];
                one[3] -= delz*fpair*v[j][2];
              } else {
                one[0] -= fpair_vec[0]*v[j][0] + fpair_vec[1]*v[j][1] + fpair_vec[2]*v[j][2];
                one[1] -= fpair_vec[0]*v[j][0];
                one[2] -= fpair_vec[1]*v[j][1];
                one[3] -= fpair_vec[2]*v[j][2];              
              }
            }
    ...
    if(inertialflag && velocityflag){
      one[0] -= f[i][0]*v[i][0] + f[i][1]*v[i][1] + f[i][2]*v[i][2];
      one[1] -= f[i][0]*v[i][0];
      one[2] -= f[i][1]*v[i][1];
      one[3] -= f[i][2]*v[i][2];
    }
...
void ComputeGroupGroupFV::kspace_contribution()
{
...
void ComputeGroupGroupFV::kspace_correction()
{
