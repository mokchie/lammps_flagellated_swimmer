...
PairSDPDCMoistureDiffv::PairSDPDCMoistureDiffv(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
PairSDPDCMoistureDiffv::~PairSDPDCMoistureDiffv()
{
  if (allocated) {
...
void PairSDPDCMoistureDiffv::compute(int eflag, int vflag)
{
  ...
  double p_i,p_j,gam,sig,fnc[3],omx,omy,omz,vir_mul,Di,Dj;
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            if (update->ntimestep*update->dt>=t0){
              for (k = 0; k < atom->individual; k++){
                if (k>0 && (((mask[i] & groupbit_solid) && conc[i][k]<conc[j][k] && !(mask[j] & groupbit_solid)) || (!(mask[i] & groupbit_solid) && conc[i][k]>conc[j][k] && (mask[j] & groupbit_solid)))) continue; //conpounds are not allowed to diffuse back to the solids
                ...
                //     f_conc[i][k] += wd * conc[i][k]*(conc[i][0]-conc[j][0])/rho[i]/rho[j]*(Df[itype][jtype][0]*rho[i]+Df[jtype][itype][0]*rho[j]);
                // }
                // else{
                //   if (mask[i] & groupbit_solid)
                //   f_conc[i][k] += -wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Df[itype][jtype][k]*(1-conc[i][k])*rho[i]+Df[jtype][itype][k]*(1-conc[j][k])*rho[j]);
                // }
                // if (newton_pair || j < nlocal) {
                //   if(k>0){
                //     f_conc[j][k] += wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Df[itype][jtype][k]*rho[i]+Df[jtype][itype][k]*rho[j]);
                //     if (mask[j] & groupbit_solid)
                //       f_conc[j][k] +=  wd * conc[i][k]*(conc[i][0]-conc[j][0])/rho[i]/rho[j]*(Df[itype][jtype][0]*rho[i]+Df[jtype][itype][0]*rho[j]);
                //   }
                //   else{
                //     if (mask[j] & groupbit_solid)
                //     f_conc[j][k] += wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Df[itype][jtype][k]*(1-conc[i][k])*rho[i]+Df[jtype][itype][k]*(1-conc[j][k])*rho[j]);
                //   }

                // }
                if(k>0 && (mask[i] & groupbit_solid || mask[j] & groupbit_solid)){
                  if (mask[i] & groupbit_solid)
                    Di = Df[itype][jtype][k] * (1+(1-epsilon0)/epsilon0*conc[i][0]);
                  else
                    Di = Df[itype][jtype][k];
                  if (mask[j] & groupbit_solid)
                    Dj = Df[jtype][itype][k] * (1+(1-epsilon0)/epsilon0*conc[j][0]);
                  else
                    Dj = Df[jtype][itype][k];

                    f_conc[i][k] += -wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Di*rho[i]+Dj*rho[j]);
                    if (newton_pair || j < nlocal) {
                      f_conc[j][k] += wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Di*rho[i]+Dj*rho[j]);
                    }
                } else {
                 f_conc[i][k] += -wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Df[itype][jtype][k]*rho[i]+Df[jtype][itype][k]*rho[j]);
                if (newton_pair || j < nlocal) {
...
void PairSDPDCMoistureDiffv::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairSDPDCMoistureDiffv::settings(int narg, char **arg)
{
  long unsigned int seed2;
  int grph;
  if (narg<12 || narg>14) error->all(FLERR,"Illegal pair_style command");
  ...
  epsilon0 = force->numeric(FLERR,arg[11]); 
  if (narg >= 13){
    if (strcmp(arg[12],"x")==0)
      bd = 1;
    else if (strcmp(arg[12],"y")==0)
      bd = 2;
    else if (strcmp(arg[12],"z")==0)
      bd = 3;
    else if (strcmp(arg[12],"xy")==0 || strcmp(arg[12],"yx")==0)
      bd = 4; 
    else if (strcmp(arg[12],"xz")==0 || strcmp(arg[12],"zx")==0)
      bd = 5; 
    else if (strcmp(arg[12],"yz")==0 || strcmp(arg[12],"zy")==0)
      bd = 6;
    else if (strcmp(arg[12],"xyz")==0 || strcmp(arg[12],"xzy")==0 || strcmp(arg[12],"yxz")==0 || strcmp(arg[12],"yzx")==0 || strcmp(arg[12],"zxy")==0 || strcmp(arg[12],"zyx")==0)
      bd = 7;     
    else if (strcmp(arg[12],"NULL")==0)
      bd = 0;
...
void PairSDPDCMoistureDiffv::coeff(int narg, char **arg)
{
...
void PairSDPDCMoistureDiffv::init_style()
{
...
double PairSDPDCMoistureDiffv::init_one(int i, int j)
{
...
void PairSDPDCMoistureDiffv::write_restart(FILE *fp)
{
...
void PairSDPDCMoistureDiffv::read_restart(FILE *fp)
{
...
void PairSDPDCMoistureDiffv::write_restart_settings(FILE *fp)
{
  ...
  fwrite(&epsilon0,sizeof(double),1,fp);
...
void PairSDPDCMoistureDiffv::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
  ...
  if (comm->me == 0) {
    ...
    fread(&epsilon0,sizeof(double),1,fp);
  ...
  MPI_Bcast(&epsilon0,1,MPI_DOUBLE,0,world);
...
double PairSDPDCMoistureDiffv::single(int i, int j, int itype, int jtype, double rsq,
...
double PairSDPDCMoistureDiffv::single_vec(int i, int j, int itype, int jtype, double rsq,
...
void PairSDPDCMoistureDiffv::set_weight()
{
...
//   nw_max = static_cast<int> (rr*dr_inv) + 2;
//   if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDCMoistureDiffv::set_weight.");
//   memory->create(weight1,n+1,n+1,nw_max,"pair:weight1");
//   memory->create(weight2,n+1,n+1,nw_max,"pair:weight2");
...
//     for (j = i; j <= n; j++){
//       k = static_cast<int> (cut[i][j]*dr_inv) + 1;
//       if (k > nw_max) error->all(FLERR,"Error in PairSDPDCMoistureDiffv::set_weight - k > nw_max");
...
void PairSDPDCMoistureDiffv::generate_wrr()
{
  int i;
  double ww[3][3];
...
int PairSDPDCMoistureDiffv::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPDCMoistureDiffv::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
int PairSDPDCMoistureDiffv::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
void PairSDPDCMoistureDiffv::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho; 
