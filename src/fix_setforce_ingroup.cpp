...
#include <cstring>
#include <cstdlib>
#include "fix_setforce_ingroup.h"
#include "atom.h"
#include "update.h"
#include "modify.h"
#include "domain.h"
...
FixSetForceInGroup::FixSetForceInGroup(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
FixSetForceInGroup::~FixSetForceInGroup()
{
  if (copymode) return;
...
int FixSetForceInGroup::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
...
void FixSetForceInGroup::setup(int vflag)
{
...
void FixSetForceInGroup::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixSetForceInGroup::post_force(int /*vflag*/)
{
  double **x = atom->x;
  double **f = atom->f;
  int *mask = atom->mask;
...
void FixSetForceInGroup::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixSetForceInGroup::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixSetForceInGroup::compute_vector(int n)
{
...
double FixSetForceInGroup::memory_usage()
{
  double bytes = 0.0;
