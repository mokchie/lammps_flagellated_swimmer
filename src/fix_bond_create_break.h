...
FixStyle(bond/create/break,FixBondCreateBreak)

#else

#ifndef LMP_FIX_BOND_CREATE_BREAK_H
#define LMP_FIX_BOND_CREATE_BREAK_H

#include "fix.h"

namespace LAMMPS_NS {

class FixBondCreateBreak : public Fix {
 public:
  FixBondCreateBreak(class LAMMPS *, int, char **);
  ~FixBondCreateBreak();
  int setmask();
  void init();
 ...
 private:
  ...
  double cutsq,fraction_b, fraction_c;
  int overflow;
  tagint lastcheck;
