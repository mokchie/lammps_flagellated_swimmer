...
PairSDPDANGM::PairSDPDANGM(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (mask[i] & groupbit){
      if (rmass)
        ...
        im = mass[type[i]];
      rho[i] += weight1[itype][itype][0]*im;
      for (jj = 0; jj < jnum; jj++) {
        j = jlist[jj];
        j &= NEIGHMASK;
        if (mask[j] & groupbit){
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            r = sqrt(rsq);
            if (r < EPSILON) continue;     // r can be 0.0 in SDPDANGM systems
            rinv = 1.0/r;
            delvx = vxtmp - v[j][0];
            delvy = vytmp - v[j][1];
            delvz = vztmp - v[j][2];
            ...
            randnum = 2.0*random->uniform()-1.0;
            gam = (2.0+dim)*eta[itype][jtype]*wd*(1.0/rho[i]/rho[i] + 1.0/rho[j]/rho[j]);
            sig = sqrt(2.0*temperature*gam)*dtinvsqrt;
            fpair = (p_i/rho[i]/rho[i] + p_j/rho[j]/rho[j])*wd; // conservative force
            fpair -= gam*dot*rinv;  // drag force 
            fpair += sig*randnum*rinv;  // random force
            fpair *= factor_dpd;
...
void PairSDPDANGM::settings(int narg, char **arg)
{
  ...
  if (grph == -1) error->all(FLERR,"Could not find group ID in sdpdangm");
  groupbit = group->bitmask[grph];
  cut_global = force->numeric(FLERR,arg[5]);
  seed = force->inumeric(FLERR,arg[6]);
  grph = group->find(arg[7]);
  if (grph == -1) error->all(FLERR,"Could not find group_rho ID in sdpdangm");
...
void PairSDPDANGM::coeff(int narg, char **arg)
{
...
void PairSDPDANGM::init_style()
{
  if (comm->ghost_velocity == 0)
    error->all(FLERR,"Pair sdpdangm requires ghost atoms store velocity");

  // if newton off, forces between atoms ij will be double computed
  // using different random numbers

  if (force->newton_pair == 0 && comm->me == 0) error->warning(FLERR,
      "Pair sdpdangm needs newton pair on for momentum conservation");
...
double PairSDPDANGM::init_one(int i, int j)
{
...
void PairSDPDANGM::write_restart(FILE *fp)
{
...
void PairSDPDANGM::read_restart(FILE *fp)
{
...
void PairSDPDANGM::write_restart_settings(FILE *fp)
{
...
void PairSDPDANGM::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDANGM::single(int i, int j, int itype, int jtype, double rsq,
...
//   if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDANGM::set_weight.");
...
//       if (k > nw_max) error->all(FLERR,"Error in PairSDPDANGM::set_weight - k > nw_max");
...
void PairSDPDANGM::generate_wrr()
{
  int i;
  double ww[3][3];
...
int PairSDPDANGM::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPDANGM::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
int PairSDPDANGM::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
void PairSDPDANGM::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
