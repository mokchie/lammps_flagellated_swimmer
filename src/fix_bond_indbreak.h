...
FixStyle(bond/indbreak,FixBondIndBreak)

#else

#ifndef LMP_FIX_BOND_INDBREAK_H
#define LMP_FIX_BOND_INDBREAK_H

#include "fix.h"

namespace LAMMPS_NS {

class FixBondIndBreak : public Fix {
 public:
  FixBondIndBreak(class LAMMPS *, int, char **);
  ~FixBondIndBreak();
  int setmask();
  void init();
  void post_integrate();
 ...
 private:
  int me,nprocs;
  int btype,seed;
  double fraction,rhoc,breakdr,breakdr2,breakr,breakrsq;
  int groupbit_rho;
