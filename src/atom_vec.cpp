...
AtomVec::AtomVec(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  mass_type = dipole_type = moment_type = 0;
  forceclearflag = 0;
  size_data_bonus = 0;
  kokkosable = 0;
  individual = atom->individual;
...
int AtomVec::pack_bond(double **buf)
{
  tagint *tag = atom->tag;
...
void AtomVec::write_bond(FILE *fp, int n, double **buf, int index)
{
  for (int i = 0; i < n; i++) {
    if (atom->individual)
      fprintf(fp,"%d " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " %g\n",
...
int AtomVec::pack_angle(double **buf)
{
  tagint *tag = atom->tag;
...
void AtomVec::write_angle(FILE *fp, int n, double **buf, int index)
{
  for (int i = 0; i < n; i++) {
    if (atom->individual)
...
int AtomVec::pack_dihedral(double **buf)
{
  tagint *tag = atom->tag;
  ...
  double **dihedral_angle = atom->dihedral_angle; 
  int nlocal = atom->nlocal;
  int newton_bond = force->newton_bond;
...
void AtomVec::write_dihedral(FILE *fp, int n, double **buf, int index)
{
  for (int i = 0; i < n; i++) {
    if (atom->individual)
      fprintf(fp,"%d " TAGINT_FORMAT " " TAGINT_FORMAT " "
              TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " %g\n",
              index,buf[i][0],buf[i][1],buf[i][2],buf[i][3],buf[i][4],buf[i][5]);
    else  
      fprintf(fp,"%d " TAGINT_FORMAT " " TAGINT_FORMAT " "
...
/* ----------------------------------------------------------------------
   prepare image variable for comm border routines 
------------------------------------------------------------------------- */

imageint AtomVec::return_image(int n, int *pbc)
{
  imageint img = atom->image[n];
  ...
  if (pbc[1]){
    idim = (img >> IMGBITS) & IMGMASK;
    otherdims = img ^ (idim << IMGBITS);
    idim -= pbc[1];
    idim &= IMGMASK;
    img = otherdims | (idim << IMGBITS);
  }

  if (pbc[2]){
    idim = img >> IMG2BITS;
    otherdims = img ^ (idim << IMG2BITS);
    idim -= pbc[2];
    idim &= IMGMASK;
    img = otherdims | (idim << IMG2BITS);
  }
  return img;
}
