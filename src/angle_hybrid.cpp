...
AngleHybrid::~AngleHybrid()
{
  ...
  if (allocated) {
    ...
    for (int i = 0; i < nstyles; i++){
      ...
      memory->destroy(anglelist_area[i]);
    }
    delete [] anglelist;
    delete [] anglelist_area;
  }
}
...
void AngleHybrid::compute(int eflag, int vflag)
{
  ...
  double *anglelist_area_orig = neighbor->anglelist_area;
  ...
  if (neighbor->ago == 0) {
    ...
    for (m = 0; m < nstyles; m++) {
      if (nanglelist[m] > maxangle[m]) {
        ...
        memory->destroy(anglelist_area[m]);
        maxangle[m] = nanglelist[m] + EXTRA;
        memory->create(anglelist[m],maxangle[m],4,"angle_hybrid:anglelist");
        memory->create(anglelist_area[m],maxangle[m],"angle_hybrid:anglelist_area");
      }
      nanglelist[m] = 0;
    }
    for (i = 0; i < nanglelist_orig; i++) {
      ...
      anglelist_area[m][n] = anglelist_area_orig[i];
      nanglelist[m]++;
    }
  }
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    neighbor->anglelist_area = anglelist_area[m];
  ...
  neighbor->anglelist_area = anglelist_area_orig;
}
...
void AngleHybrid::allocate()
{
  ...
  anglelist_area = new double*[nstyles];
  for (int m = 0; m < nstyles; m++) maxangle[m] = 0;
  for (int m = 0; m < nstyles; m++) {
    anglelist[m] = NULL;
    anglelist_area[m] = NULL;
  }
}
