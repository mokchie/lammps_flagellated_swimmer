  ...
  xstr(NULL), ystr(NULL), zstr(NULL), estr(NULL), idregion(NULL), sforce(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"every") == 0) {
      ...
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix addforce command");
...
void FixAddForce::init()
{
  ...
  if (adapt_ind && varflag != CONSTANT)
    error->all(FLERR,"Cannot use adapt function with "
               "variable force in fix addforce");
...
{ 
  ...
  if (comm->me == 0){
    ff[2] = 0.0;
    j = static_cast<int>(universe->me/23.0) + 1;
    ...
    fclose(f_read);
    ff[1] = yvalue - vv; 
    if (logfile) fprintf(logfile,"y: j = %d, niter = %d, f = %f, %f; vv = %f \n",j,niter,yvalue,ff[1],vv);
