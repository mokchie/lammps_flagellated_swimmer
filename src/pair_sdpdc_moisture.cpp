...
PairSDPDCMoisture::PairSDPDCMoisture(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
PairSDPDCMoisture::~PairSDPDCMoisture()
{
  if (allocated) {
...
void PairSDPDCMoisture::compute(int eflag, int vflag)
{
...
void PairSDPDCMoisture::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairSDPDCMoisture::settings(int narg, char **arg)
{
...
void PairSDPDCMoisture::coeff(int narg, char **arg)
{
...
void PairSDPDCMoisture::init_style()
{
...
double PairSDPDCMoisture::init_one(int i, int j)
{
...
void PairSDPDCMoisture::write_restart(FILE *fp)
{
...
void PairSDPDCMoisture::read_restart(FILE *fp)
{
...
void PairSDPDCMoisture::write_restart_settings(FILE *fp)
{
...
void PairSDPDCMoisture::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDCMoisture::single(int i, int j, int itype, int jtype, double rsq,
...
double PairSDPDCMoisture::single_vec(int i, int j, int itype, int jtype, double rsq,
...
void PairSDPDCMoisture::set_weight()
{
...
//   nw_max = static_cast<int> (rr*dr_inv) + 2;
//   if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDCMoisture::set_weight.");
//   memory->create(weight1,n+1,n+1,nw_max,"pair:weight1");
//   memory->create(weight2,n+1,n+1,nw_max,"pair:weight2");
...
//     for (j = i; j <= n; j++){
//       k = static_cast<int> (cut[i][j]*dr_inv) + 1;
//       if (k > nw_max) error->all(FLERR,"Error in PairSDPDCMoisture::set_weight - k > nw_max");
...
void PairSDPDCMoisture::generate_wrr()
{
  int i;
  double ww[3][3];
...
int PairSDPDCMoisture::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPDCMoisture::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
int PairSDPDCMoisture::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
void PairSDPDCMoisture::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho; 
