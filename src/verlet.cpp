...
void Verlet::setup(int flag)
{
  ...
  //printf("domain->image_check");
  domain->image_check();
  //printf("domain->box_too_small_check");
  domain->box_too_small_check();
  //printf("modify->setup_pre_neighbor");
  modify->setup_pre_neighbor();
  //printf("neighbor->build(1)\n");
  neighbor->build(1);
  modify->setup_post_neighbor();
  neighbor->ncalls = 0;

  // compute all forces
  //printf("compute all force\n");
  force->setup();
...
void Verlet::run(int n)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    // all output
    //next calculation for statistic
    if(ntimestep == output->next_calc) {
      output->calc(ntimestep);
    }
