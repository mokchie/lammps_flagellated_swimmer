...
AngleCosineSquaredEqvar::AngleCosineSquaredEqvar(LAMMPS *lmp) : Angle(lmp)
{
  k = NULL;
  b = NULL;
  omega = NULL;
...
AngleCosineSquaredEqvar::~AngleCosineSquaredEqvar()
{
  if (allocated) {
...
void AngleCosineSquaredEqvar::compute(int eflag, int vflag)
{
  int i1,i2,i3,n,type;
  double delx1,dely1,delz1,delx2,dely2,delz2;
  double eangle,f1[3],f3[3];
  double dcostheta,tk,thx;
  ...
  for (n = 0; n < nanglelist; n++) {
    ...
    dcostheta = c - cos( theta0[type] + b[type]*sin(thx + omega[type]*update->ntimestep*update->dt) );
    tk = k[type] * dcostheta;
...
void AngleCosineSquaredEqvar::allocate()
{
  allocated = 1;
...
void AngleCosineSquaredEqvar::coeff(int narg, char **arg)
{
...
double AngleCosineSquaredEqvar::equilibrium_angle(int i)
{
  return theta0[i];
}
...
void AngleCosineSquaredEqvar::write_restart(FILE *fp)
{
...
void AngleCosineSquaredEqvar::read_restart(FILE *fp)
{
  allocate();
...
void AngleCosineSquaredEqvar::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nangletypes; i++)
    fprintf(fp,"%d %g %g %g %g\n",i,k[i],b[i]/MY_PI*180.0,omega[i],theta0[i]/MY_PI*180.0);
}

/* ---------------------------------------------------------------------- */

double AngleCosineSquaredEqvar::single(int type, int i1, int i2, int i3)
{
  double **x = atom->x;
