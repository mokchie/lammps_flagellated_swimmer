...
FixWallMeasure::FixWallMeasure(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  nwall(0)
{
  scalar_flag = 1;
...
void FixWallMeasure::setup(int vflag)
{
...
void FixWallMeasure::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixWallMeasure::pre_force(int vflag)
{
  post_force(vflag);
}
...
void FixWallMeasure::post_force(int vflag)
{
  eflag = 0;
  for (int m = 0; m <= nwall; m++) ewall[m] = 0.0;
  ...
  for (int m = 0; m < nwall; m++) {
    ...
    total_force = wall_particle(m,wallwhich[m],coord);
  }
...
void FixWallMeasure::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixWallMeasure::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixWallMeasure::compute_scalar()
{
...
double FixWallMeasure::compute_vector(int n)
{
  // only sum across procs one time
