...
void CreateAtoms::add_single()
{
  ...
  if (comm->user_part){
    ind = comm->coords_to_bin(coord,nn,comm->nbp_loc,comm->box_min);  // needs to be changed for adding molecules
    if (ind)
      ...
      for (i = ilo; i <= ihi; i++)
        for (m = 0; m < nbasis; m++) {
          ...
          if (comm->user_part){
            ...
            if (ind)
              if (comm->m_comm[nn[0]][nn[1]][nn[2]] & comm->local_bit) continue; 
          } else {
            if (coord[0] < sublo[0] || coord[0] >= subhi[0] ||
                coord[1] < sublo[1] || coord[1] >= subhi[1] ||
