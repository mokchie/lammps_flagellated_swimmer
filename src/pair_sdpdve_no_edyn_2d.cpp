...
PairSDPDVENoEdyn2D::PairSDPDVENoEdyn2D(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
PairSDPDVENoEdyn2D::~PairSDPDVENoEdyn2D()
{
  if (allocated) {
...
void PairSDPDVENoEdyn2D::compute(int eflag, int vflag)
{
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            r = sqrt(rsq);
            if (r < EPSILON) continue;     // r can be 0.0 in SDPDVENoEdyn2DNoEdyn2D systems
            rinv = 1.0/r;
            delvx = vxtmp - v[j][0];
            delvy = vytmp - v[j][1];
            delvz = 0.0;
...
void PairSDPDVENoEdyn2D::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairSDPDVENoEdyn2D::settings(int narg, char **arg)
{
...
void PairSDPDVENoEdyn2D::coeff(int narg, char **arg)
{
...
void PairSDPDVENoEdyn2D::init_style()
{
...
double PairSDPDVENoEdyn2D::init_one(int i, int j)
{
...
void PairSDPDVENoEdyn2D::write_restart(FILE *fp)
{
...
void PairSDPDVENoEdyn2D::read_restart(FILE *fp)
{
...
void PairSDPDVENoEdyn2D::write_restart_settings(FILE *fp)
{
...
void PairSDPDVENoEdyn2D::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDVENoEdyn2D::single(int i, int j, int itype, int jtype, double rsq,
...
// void PairSDPDVENoEdyn2D::set_weight()
// {
...
//   /*  printf("\n########################\n");
//     for (l=0; l<nw_max; l++){
//       printf("%f %f %f\n",l*DR,weight1[1][1][l],weight2[1][1][l]);
//     }
//         printf("\n########################\n");
//         */
// }
...
void PairSDPDVENoEdyn2D::generate_wrr()
{
  int i;
  double ww[3][3];
...
int PairSDPDVENoEdyn2D::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPDVENoEdyn2D::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
int PairSDPDVENoEdyn2D::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
void PairSDPDVENoEdyn2D::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
