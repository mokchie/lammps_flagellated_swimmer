...
void Molecule::read(int flag)
{
  ...
  while (1) {
    ...
    if (strstr(line,"atoms")) {
      ...
      nwant = 1;
    } else if (strstr(line,"individual")) {
      nmatch = sscanf(line,"%d",&individual);
      nwant = 1;
  ...
  if ((atom->individual && individual == 0) || (atom->individual == 0 && individual))
     error->all(FLERR,"Individual parameter in molecule command does not match atom->individual or molecule is defined before atom_style");
...
/* ----------------------------------------------------------------------
   read types and coords from file
...
void Molecule::coords(char *line)
{
  ...
  for (int i = 0; i < natoms; i++) {
    readline(line);
    if (5 != sscanf(line,"%d %d %lg %lg %lg",&tmp,&type[i],&x[i][0],&x[i][1],&x[i][2]))
...
void Molecule::bonds(int flag, char *line)
{
  ...
  for (int i = 0; i < nbonds; i++) {
    readline(line);
    if (individual){
      if (5 != sscanf(line,"%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " %lg",
...
void Molecule::angles(int flag, char *line)
{
  ...
  for (int i = 0; i < nangles; i++) {
    readline(line);
    if (individual){
      if (6 != sscanf(line,"%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " %lg",
...
void Molecule::dihedrals(int flag, char *line)
{
  ...
  for (int i = 0; i < ndihedrals; i++) {
    readline(line);
    if (individual){
      if (7 != sscanf(line,"%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " "
              TAGINT_FORMAT " " TAGINT_FORMAT " %lg",
...
void Molecule::allocate()
{
  ...
  if (bondflag) {
    ...
    memory->create(bond_atom,natoms,bond_per_atom,
                   "molecule:bond_atom");
    if (individual)
      memory->create(bond_length,natoms,bond_per_atom,
  ...
  if (angleflag) {
    ...
    if (individual)
      memory->create(angle_area,natoms,angle_per_atom,
                     "molecule:angle_area");
  }
  ...
  if (dihedralflag) {
    ...
    if (individual)
      memory->create(dihedral_angle,natoms,dihedral_per_atom,
...
void Molecule::print()
{
  ...
  if (bondflag) {
    ...
    for (int i = 0; i < natoms; i++) {
      ...
      for (int j = 0; j < num_bond[i]; j++)
        if (individual)
          printf("      %d %d %d %d %g\n",j+1,bond_type[i][j],i+1,bond_atom[i][j],bond_length[i][j]);
        else 
    ...
    for (int i = 0; i < natoms; i++) {
      ...
      for (int j = 0; j < num_angle[i]; j++)
        if (individual)
          printf("      %d %d %d %d %d %g\n",
               j+1,angle_type[i][j],
               angle_atom1[i][j],angle_atom2[i][j],angle_atom3[i][j],angle_area[i][j]); 
        else 
          printf("      %d %d %d %d %d\n",
               j+1,angle_type[i][j],
      ...
      for (int j = 0; j < num_dihedral[i]; j++)
        if (individual)
          printf("      %d %d %d %d %d %d %g\n",
               ...
               dihedral_atom3[i][j],dihedral_atom4[i][j],dihedral_angle[i][j]);
        else 
          printf("      %d %d %d %d %d %d\n",
               j+1,dihedral_type[i][j],
               dihedral_atom1[i][j],dihedral_atom2[i][j],
