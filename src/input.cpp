...
int Input::execute_command()
{
  ...
  else if (!strcmp(command,"moment")) moment();
  ...
  else if (!strcmp(command,"statistic")) statistic();
...
{
  if (narg < 2) error->all(FLERR,"Illegal atom_style command");
  if (domain->box_exist)
...
void Input::comm_style()
{
  ...
  if (strcmp(arg[0],"brick") == 0) {
    ...
    delete oldcomm;
  } else if (strcmp(arg[0],"mosaic") == 0) {
    if (narg < 2) error->all(FLERR,"Illegal comm_style mosaic command");  
    if (comm->style == 2) return;
    Comm *oldcomm = comm;
    comm = new CommMosaic(lmp,oldcomm); 
    delete oldcomm;
    comm->user_part = 1; 
    delete [] comm->customfile;
    int n = strlen(arg[1]) + 1;
    comm->customfile = new char[n];
    strcpy(comm->customfile,arg[1]);
...
void Input::moment()
{
  if (narg != 2) error->all(FLERR,"Illegal moment command");
  if (domain->box_exist == 0)
    error->all(FLERR,"Moment command before simulation box is defined");
  atom->set_moment(FLERR,narg,arg);
}
...
void Input::statistic()
{
  output->add_statistic(narg,arg);
}
