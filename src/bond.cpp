...
/* ----------------------------------------------------------------------
   tally energy and virial into global and per-atom accumulators for wlc/pow/all/visc
------------------------------------------------------------------------- */

void Bond::ev_tally2(int i, int j, int nlocal, int newton_bond,
                    double ebond, double fbond,double *fr, 
                    double delx, double dely, double delz)
{
  double ebondhalf,v[6];
  ...
  if (vflag_either) {
    v[0] = delx*(delx*fbond + fr[0]);
    v[1] = dely*(dely*fbond + fr[1]);
    v[2] = delz*(delz*fbond + fr[2]);
    v[3] = delx*(dely*fbond + fr[1]);
    v[4] = delx*(delz*fbond + fr[2]);
    v[5] = dely*(delz*fbond + fr[2]);
...
/* ----------------------------------------------------------------------
   tally energy and virial into global and per-atom accumulators for area/wlc/pow/visc
------------------------------------------------------------------------- */

void Bond::ev_tally3(int i, int j, int nlocal, int newton_bond,
                    double ebond, double **fa,double *fr,double fbond, double vv,
                    double **xx, double delx, double dely, double delz)
{
  double ebondhalf,v[6];
  ...
  if (vflag_either) {
    v[0] = delx*(delx*fbond + fr[0]) - 0.5*(fa[1][0]+fa[0][0])*(xx[1][0]-xx[0][0]) - vv;
    v[1] = dely*(dely*fbond + fr[1]) - 0.5*(fa[1][1]+fa[0][1])*(xx[1][1]-xx[0][1]) - vv;
    v[2] = delz*(delz*fbond + fr[2]);
    v[3] = delx*(dely*fbond + fr[1]) - 0.5*(fa[1][1]+fa[0][1])*(xx[1][0]-xx[0][0]);
    v[4] = delx*(delz*fbond + fr[2]);
    v[5] = dely*(delz*fbond + fr[2]);
