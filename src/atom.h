...
class Atom : protected Pointers {
 public:
  ...
  tagint n_mol_max;             // max # of molecules in system
  int n_mol_types;              // # of molecule types based on molecule size
  int nmax;                     // max # of owned+ghost in arrays on this proc
  int tag_enable;               // 0/1 if atom ID tags are defined
  int molecular;                // 0 = atomic, 1 = standard molecular system,
                                // 2 = molecule template system
  int individual;               // 0 = old, 1 = with individual bonds, angles, dihedrals, 2 = also indiviual but with two individual parameters for every angles
  double c_v;
  int mol_corresp_ind, fix_force_bound_ind;
  tagint n_mol_corresp_glob, n_mol_corresp_glob_max, *mol_corresp_glob;
  ...
  int *mol_type;            // molecule type based on molecule size
  tagint *mol_size;         // sizes of molecule types 
  tagint *molecule;
  ...
  double ***ctensor;
  double ***kappa; // kappa is the velocity gradient tensor at positons of the particles
  double ***chi;
  double **ctensor_eigenval, ***ctensor_eigenvec; //ctensor: conformation tensor
  double **f_ctensor_eigenval, ***f_ctensor_eigenvec;
  double ***f_ctensor;
  ...
  double *rho,*drho,*e,*de,*cv,*solid_rho,*fluid_rho;
  double **vest;
  double *pte0,*pte1;
  ...
  double *mass,*moment;
  int *mass_setflag,*moment_setflag;
  ...
  void set_moment(const char *, int, const char *, int);
  void set_moment(const char *, int, int, double);
  void set_moment(const char *, int, int, char **);
  void set_moment(double *);
  void check_moment(const char *, int);
