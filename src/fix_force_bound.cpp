...
FixForceBound::FixForceBound(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  if (me == 0){
    ...
    fgets(buf,BUFSIZ,f_read);
    sscanf(buf,"%d %lf %lf %lf %d %d %d %s %s %s",&ind_bounce,&d_cut,&comm_cut,&binsize,&max_count,&cell_update,&build_tri_delay,&grp[0],&grp1[0],&grp2[0]);
    ...
    rdata[l++] = static_cast<double> (build_tri_delay);
    igroup = group->find(grp);
  ...
  if (me)
    memory->create(rdata,l,"fix_force_bound:rdata");
  ...
  build_tri_delay = static_cast<int> (rdata[l++]);    
  ...
  rot = NULL;
  ndiv = NULL;
  ptype = refl = face_order = shapes_local = shapes_global_to_local = NULL;
  ...
  memory->create(part_loc_map,nnmax,"fix_force_bound:part_loc_map");
}
...
{
  int i,j,k,m,kk,ind,cond,i_x[NUM2],cc,bounce,cplane0,cplane1; 
  int ccount,i0,i1,i2,m0,i_b[NUM2];
  double dl[3],dl1[3],t_x[NUM2],dd[3],xp[3],xh[3],vh[3],vh1[3],vv[3],norm_h[3],x_x[3]; 
  double xd[3],xc[3],cf[4],a1[3],a2[3],nnp[4],v_loc[3],xp1[3],shift[NUM2][3];
  ...
  for (i = 0; i < nlocal; i++)
        ...
        if (mask[i] & groupbit){
          ...
          for (k = 0; k < num_faces[i]; k++){
            ...
            if (m0 != cplane0){
              ...
              i0 = part_part_loc[tri_part[m0][0]];
              m = norm_ind[m0]; 
              for (j = 0; j < 3; j++){
              ...
              u0 = nnp[0]*(xp1[0]+shift[ind][0]) + nnp[1]*(xp1[1]+shift[ind][1]) + nnp[2]*(xp1[2]+shift[ind][2]) + nnp[3];
              if ((u0 <= 0.0 && mask[i] & groupbit_inner) || (u0 >= 0.0 && !(mask[i] & groupbit_inner))){
                u1 = norm[m][0]*(x[i][0]+shift[ind][0]) + norm[m][1]*(x[i][1]+shift[ind][1]) + norm[m][2]*(x[i][2]+shift[ind][2]) + norm[m][3];
                if (u0*u1 <= 0.0){ 
                  for (j = 0; j < 3; j++){
	...
	if (mask[i] & groupbit_solid){
	  ...
	  while (ind){
	      ...
	      m0 = i_x[m];
              i0 = part_part_loc[tri_part[m0][0]];
              i1 = part_part_loc[tri_part[m0][1]];
              i2 = part_part_loc[tri_part[m0][2]];
              kk = norm_ind[m0];
              for (k = 0; k < 3; k++){
...
void FixForceBound::post_force(int vflag)
{
  ...
  if (nlocal > nnmax){
    ...
    memory->grow(part_loc_map,nnmax,"fix_force_bound:part_loc_map");
  }  
  ...
  if (neighbor->ago == 0){
    ...
    norm_count = 0;  
    shape_decide();
    if (build_tri_next >= build_tri_delay || atom->fix_force_bound_ind) build_tri_lists();
    local_faces(0);
    ...
    for (i = 0; i < nlocal; i++){
      if (ind_press_cell && mask[i] & groupbit_p_cell)
        for (m = 0; m < num_faces[i]; m++){
          ...
          ix = part_part_loc[tri_part[m0][0]];
          u1 = - norm[k][0]*x_part[ix][0] - norm[k][1]*x_part[ix][1] - norm[k][2]*x_part[ix][2]; 
          rr = u1 - u0;
          hh = fabs(rr);
          if (hh < r_press){
            iy = part_part_loc[tri_part[m0][1]];
            iz = part_part_loc[tri_part[m0][2]];
...
void FixForceBound::build_tri_lists()
{
  int i, j, l, m, max_tri, nsend, nrecv, ind;
  int iswap, dim, ineed, sendflag, twoneed, nfirst, nlast, nm1, nm2;
  tagint k;
  tagint **tri_tags = NULL;
  tagint *tag = atom->tag; 
  int *mask = atom->mask;
  ...
  if (max_ntri_tot > 0)
    memory->create(tri_tags,max_ntri_tot,3,"fix_force_bound:tri_tags");

  atom->fix_force_bound_ind = 0;
  build_tri_next = 0;
  ntri_tot = 0;
  for (i = 0; i < nanglelist; i++){
    if (ntri_tot == max_ntri_tot){
      grow_tri_lists();
      memory->grow(tri_tags,max_ntri_tot,3,"fix_force_bound:tri_tags");
    }
    if (mask[anglelist[i][1]] & groupbit_comm){
      for (l = 0; l < 3; l++)
        tri_tags[ntri_tot][l] = tag[anglelist[i][l]];
      tri_mol[ntri_tot] = molecule[anglelist[i][0]];
      tri_type[ntri_tot] = type[anglelist[i][0]];
      ntri_tot++;
    }     
  }
  ...
  if (user_part){
    ...
    nsend = ntri_tot;
    if (nsend*size_tri > maxsend) grow_send(nsend*size_tri);

    // pack triangles to send to neighbors
    m = 0;
    for (i = 0; i < ntri_tot; i++){
      for (l = 0; l < 3; l++)
        buf_send[m++] = ubuf(tri_tags[i][l]).d;
      buf_send[m++] = ubuf(tri_mol[i]).d;
      buf_send[m++] = ubuf(tri_type[i]).d;
    }     

    for (dim = 0; dim < n_neigh; dim++) {

      // swap triangles with other proc
      // no MPI calls except SendRecv if nsend/nrecv = 0

      MPI_Sendrecv(&nsend,1,MPI_INT,sendproc[dim],me,
                   &nrecv,1,MPI_INT,recvproc[dim],recvproc[dim],world,MPI_STATUS_IGNORE);
      if (nrecv*size_tri > maxrecv) grow_recv(nrecv*size_tri);
      if (nrecv) MPI_Irecv(buf_recv,nrecv*size_tri,MPI_DOUBLE,
                           recvproc[dim],recvproc[dim],world,&request);
      if (nsend) MPI_Send(buf_send,nsend*size_tri,MPI_DOUBLE,sendproc[dim],me,world);
      if (nrecv) MPI_Wait(&request,MPI_STATUS_IGNORE);

      // unpack buffer to the end of array
      m = 0;
      for (i = 0; i < nrecv; i++){
        if (ntri_tot == max_ntri_tot){ 
          grow_tri_lists();
        ...
        for (l = 0; l < 3; l++)
          tri_tags[ntri_tot][l] = (tagint) ubuf(buf_recv[m++]).i;
        tri_mol[ntri_tot] = (tagint) ubuf(buf_recv[m++]).i;
        tri_type[ntri_tot] = (int) ubuf(buf_recv[m++]).i;
        ntri_tot++;
      }
    }
    ...
    for (dim = 0; dim < 3; dim++) {
      ...
      for (ineed = 0; ineed < twoneed; ineed++) {
        ...
        ind = 1;
        if (domain->periodicity[dim] && ineed > comm->procgrid[dim]-2) 
          ind = 0; 
        ...
        if (ind){
          ...
          // swap triangles with other proc
          // put incoming ghosts at end of my triangle arrays
 	    ...
 	    if (nrecv*size_tri > maxrecv) grow_recv(nrecv*size_tri);
	    if (nrecv) MPI_Irecv(buf_recv,nrecv*size_tri,MPI_DOUBLE,recvproc[iswap],0,world,&request);
	    if (nsend) MPI_Send(buf_send,nsend*size_tri,MPI_DOUBLE,sendproc[iswap],0,world);
	    if (nrecv) MPI_Wait(&request,MPI_STATUS_IGNORE);
          } else {
            nsend = 0;
            nrecv = 0;
  ...
  if (ntri_tot){
    ...
    tags_tot = max_ind - min_ind + 1;
    if (tags_tot > max_tags_tot){
      max_tags_tot = tags_tot + NUM1;
      memory->destroy(tags_part);
      memory->create(tags_part,max_tags_tot,"fix_force_bound:tags_part");
    }

    for (k = 0; k < tags_tot; k++)
      tags_part[k] = -1;
    ...
    for (i = 0; i < ntri_tot; i++){
      ...
      for (j = 0; j < 3; j++){
        ...
        k = tri_tags[i][j];
        if (tags_part[k] == -1){ 
          tags_part[k] = npart_tot;
          npart_tot++; 
        }
      }
    }

    if (npart_tot > max_npart_tot) grow_part_lists();
    ...
    if (max_tri > max_tri_part){
      max_tri_part = max_tri;
      memory->destroy(part_tri);
      memory->create(part_tri,max_npart_tot,max_tri_part,"fix_force_bound:part_tri");  
    }
  ...
  fprintf(out_stat,"ntri_tot=%d, max_tri_part=%d  \n", ntri_tot, max_tri_part); 
  fprintf(out_stat,"triangles  \n"); 
  for (i = 0; i < ntri_tot; i++)
    fprintf(out_stat,"%d    %ld %ld %ld    %d %d %d    %d %d \n",i,tri_tags[i][0],tri_tags[i][1],tri_tags[i][2],tri_part[i][0],tri_part[i][1],tri_part[i][2],tri_mol[i],tri_type[i]);
	...
	if (m > -1 && m < tags_tot)
        ...
        if (k == -1) { 
          if (atom->mol_corresp_ind)
            ...
            error->one(FLERR,"Could not find a local comm particle in the pre-built list. Probably the frequency of list re-building should be increased.");   
        }
        part_loc_map[i] = npart;
        part_list[npart] = k;
        part_part_loc[k] = npart;
        for (j = 0; j < 3; j++)
...
void FixForceBound::grow_tri_lists()
{
  max_ntri_tot += BUFEXTRA;
  memory->grow(tri_mol,max_ntri_tot,"fix_force_bound:tri_mol");
  memory->grow(tri_type,max_ntri_tot,"fix_force_bound:tri_type");
  memory->destroy(tri_part);
  memory->create(tri_part,max_ntri_tot,3,"fix_force_bound:tri_part");
  memory->destroy(norm_ind);
  memory->create(norm_ind,max_ntri_tot,"fix_force_bound:norm_ind"); 
}
...
void FixForceBound::grow_part_lists()
{
  max_npart_tot = npart_tot + NUM1;
  memory->destroy(part_degree);
  memory->create(part_degree,max_npart_tot,"fix_force_bound:part_degree");
  memory->destroy(part_tags);
  memory->create(part_tags,max_npart_tot,"fix_force_bound:part_tags");
  memory->destroy(part_tri);
  memory->create(part_tri,max_npart_tot,max_tri_part,"fix_force_bound:part_tri");
  memory->destroy(part_part_loc);
  memory->create(part_part_loc,max_npart_tot,"fix_force_bound:part_part_loc");
...
int FixForceBound::pack_border_h(int i, double *buf, int pbc_flag, int *pbc)
{
  ...
  buf[m++] = ubuf(part_tags[part_list[i]]).d;
  if (pbc_flag == 0) {
    buf[m++] = x_part[i][0];
    buf[m++] = x_part[i][1];
    buf[m++] = x_part[i][2];
...
int FixForceBound::unpack_border_h(double *buf)
{
  ...
  if (npart == max_npart) grow_part_arrays();
  k = (tagint) ubuf(buf[m++]).i;
  if (k < min_ind || k > max_ind){ 
    error->warning(FLERR,"The tag value is out of bounds in unpack_border_h! This particle will be omitted! A higher frequency of list re-building may resolve this issue.");  
    return 4;
  }
  part_list[npart] = tags_part[k - min_ind];
  if (part_list[npart] < 0 || part_list[npart] >= npart_tot){
    error->warning(FLERR,"Did not find the particle in local lists in unpack_border_h! This particle will be omitted! A higher frequency of list re-building may resolve this issue."); 
    return 4;  
  }
  part_part_loc[part_list[npart]] = npart;
...
/*void FixForceBound::face_decide()
{
  ...
  for (jj = 0; jj < npart; jj++){
    ...
    for (n = 0; n < part_degree[j]; n++){
      k = part_tri[j][n];
      if (norm_ind[k] == -1 && part_part_loc[tri_part[k][0]] > -1 &&  part_part_loc[tri_part[k][1]] > -1 && part_part_loc[tri_part[k][2]] > -1) {
...
void FixForceBound::face_decide()
{
  int i,j,k,m,n,jj,kk,bin,h_bin,ii,ind_h;
  double dd[3],rr;
	      ...
	      if (rr < d_cut_sq)
                for (n = 0; n < part_degree[j]; n++){
                  ...
                  if (norm_ind[k] == -1 && part_part_loc[tri_part[k][0]] > -1 &&  part_part_loc[tri_part[k][1]] > -1 && part_part_loc[tri_part[k][2]] > -1) {
                    ...
                    if (norm_ind[k] == -1)
		      if (!atom->mol_corresp_ind) 
                        printf("It might make sense to increase comm_cut, because some triangles are omitted!!! me=%d, part_loc=%d %d %d \n",me,part_part_loc[tri_part[k][0]],part_part_loc[tri_part[k][1]],part_part_loc[tri_part[k][2]]);
	  	  }
                }
            }
          }
...
void FixForceBound::calc_norm(int m)
{
  ...
  i0 = part_part_loc[tri_part[m][0]];
  i1 = part_part_loc[tri_part[m][1]];
  i2 = part_part_loc[tri_part[m][2]]; 
  for (k = 0; k < 3; k++){
...
void FixForceBound::move_norm_arrays(int m)
{
  int k,i;
