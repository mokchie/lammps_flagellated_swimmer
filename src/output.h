...
class Output : protected Pointers {
 public:
  ...
  int max_stat;
  bigint next_calc;
  bigint next_stat_write_any;
  int *every_stat_write, *stress_id;
  int *every_stat_calc;
  bigint *start_calc;
  bigint *next_stat_calc;
  bigint *next_stat_write;
  bigint *last_stat_calc;
  bigint *last_stat_write;
  ...
  void calc(bigint);                 // calculates statistic
  ...
  void add_statistic(int, char **);  // add tatistic to satistic list
