  ...
  group2(NULL)
{
  ...
  if (strcmp(arg[3],"tether") == 0) {
    ...
    ind_calc_com = force->inumeric(FLERR,arg[9]);
  ...
  } else if (strcmp(arg[3],"oscillate") == 0) {
    if (narg != 13) error->all(FLERR,"Illegal fix spring command");
    styleflag = OSCILLATE;
    k_spring = force->numeric(FLERR,arg[4]);
    xflag = yflag = zflag = 1;
    if (strcmp(arg[5],"NULL") == 0) xflag = 0;
    ...
    if (r0 < 0) error->all(FLERR,"R0 < 0 for fix spring command");
    ind_calc_com = force->inumeric(FLERR,arg[9]);
    ind_dir = force->inumeric(FLERR,arg[10]);
    amp = force->numeric(FLERR,arg[11]);
...
void FixSpring::init()
{
  ...
  if (styleflag == TETHER || styleflag == OSCILLATE)
    if (ind_calc_com){
      ...
      if (comm->me == 0)
        if (logfile) {
          fprintf(logfile,"Spring center: cx = %20.16g, cy = %20.16g, cz = %20.16g \n",xcm[0],xcm[1],xcm[2]);
          fflush(logfile);
        }
    }
  if (styleflag == OSCILLATE){
    if (ind_dir == 1) c0 = xc;
    if (ind_dir == 2) c0 = yc;
    if (ind_dir == 3) c0 = zc; 
  }
...
void FixSpring::post_force(int /*vflag*/)
{
  ...
  else if (styleflag == COUPLE) spring_couple();
  else spring_oscillate(); 
}
...
void FixSpring::spring_oscillate()
{
  double xcm[3];
  ...
  if (ind_dir == 1) xc = c0 + amp*sin(2.0*M_PI*time*omega);
  if (ind_dir == 2) yc = c0 + amp*sin(2.0*M_PI*time*omega);
  if (ind_dir == 3) zc = c0 + amp*sin(2.0*M_PI*time*omega);
