...
class Comm : protected Pointers {
 public:
  int style;     // comm pattern: 0 = 6-way stencil, 1 = irregular tiling, 2 = mosaic tiling
  ...
  // added stuff ---------------------------------------------
  int user_part, processed_data_ind;
  bigint local_bit;
  int nbp_min[3], nbp_max[3], bin_ext[3], nbp_loc[3], nbp_orig[3];
  double bin_size[3], box_min[3];
  int le;          // lees-edwards BCs 
  double u_le, shift;               // lees-edwards BCs
  bigint ***m_comm;

  virtual void lees_edwards(int){};           // lees-edwards
  virtual void read_user_file(){};
  virtual void process_bin_data(int &, int &, int *, int *, int *, int *, double *, double *){};
  virtual void build_comm_matrix(int, bigint ***, bigint *, int *, int *, int *, int *, int *, double *, int *, int *, int *, int **, int){};
  virtual int coords_to_bin(double *, int *, int *, double *){return 0;};
  // ---------------------------------------------------------
