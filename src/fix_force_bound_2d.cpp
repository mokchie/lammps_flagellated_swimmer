...
FixForceBound2D::FixForceBound2D(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  if (me == 0){
    ...
    fgets(buf,BUFSIZ,f_read);
    sscanf(buf,"%d %lf %lf %lf %d %d %d %s %s %s",&ind_bounce,&d_cut,&comm_cut,&binsize,&max_count,&cell_update,&build_bond_delay,&grp[0],&grp1[0],&grp2[0]);
    ...
    rdata[l++] = static_cast<double> (build_bond_delay);
    igroup = group->find(grp);
  ...
  build_bond_delay = static_cast<int> (rdata[l++]);
  ...
  part_list = part_part_loc = bond_type = NULL;
  part_bond = bond_part = NULL;
  bond_mol = part_tags = NULL;
  ...
  memory->create(part_loc_map,nnmax,"fix_force_bound_2d:part_loc_map");  
}
...
void FixForceBound2D::setup(int vflag)
{
  ...
  // initialize the bond structures
  max_nbond_tot = 0;
  max_tags_tot = 0;
  max_npart_tot = 0;
  max_bond_part = 2;
      ...
      while (cond && ccount < max_count){
        ...
        if (mask[i] & groupbit){
          ...
          for (k = 0; k < num_faces[i]; k++){
            ...
            m0 = ind_faces[i][k];
            if (m0 != cplane0){
              i0 = part_part_loc[bond_part[m0][0]];
              i1 = part_part_loc[bond_part[m0][1]];
              for (j = 0; j < 2; j++){
              ...
              if (mask[i] & groupbit_inner){//((u0 <= 0.0 && mask[i] & groupbit_inner) || (u0 >= 0.0 && !(mask[i] & groupbit_inner))){
...
void FixForceBound2D::post_force(int vflag)
{
  ...
  if (nlocal > nnmax){
    ...
    memory->grow(part_loc_map,nnmax,"fix_force_bound_2d:part_loc_map");
  }  
 
  if (neighbor->ago == 0){
    shape_decide();
    if (build_bond_next >= build_bond_delay || atom->fix_force_bound_ind) build_bond_lists();
    local_faces(0);
...
void FixForceBound2D::build_bond_lists()
{
  int i, j, l, m, max_bond, nsend, nrecv, ind;
  int iswap, dim, ineed, sendflag, twoneed, nfirst, nlast, nm1, nm2;
  tagint k;
  tagint **bond_tags = NULL;
  tagint *tag = atom->tag;
  int *mask = atom->mask;
  ...
  if (max_nbond_tot > 0)
    memory->create(bond_tags,max_nbond_tot,2,"fix_force_bound_2d:bond_tags");

  atom->fix_force_bound_ind = 0;
  build_bond_next = 0;
  nbond_tot = 0;
  for (i = 0; i < nbondlist; i++){
    if (nbond_tot == max_nbond_tot){
      grow_bond_lists();
      memory->grow(bond_tags,max_nbond_tot,2,"fix_force_bound_2d:bond_tags");
    }
    if (mask[bondlist[i][0]] & groupbit_comm){
      for (l = 0; l < 2; l++)
        bond_tags[nbond_tot][l] = tag[bondlist[i][l]];
      bond_mol[nbond_tot] = molecule[bondlist[i][0]];
      bond_type[nbond_tot] = type[bondlist[i][0]];
      nbond_tot++;
    }
  }
  ...
  if (user_part){
    ...
    nsend = nbond_tot;
    if (nsend*size_bond > maxsend) grow_send(nsend*size_bond);

    // pack bonds to send to neighbors
    m = 0;
    for (i = 0; i < nbond_tot; i++){
      for (l = 0; l < 2; l++)
        buf_send[m++] = ubuf(bond_tags[i][l]).d;
      buf_send[m++] = ubuf(bond_mol[i]).d;
      buf_send[m++] = ubuf(bond_type[i]).d;
    }

    for (dim = 0; dim < n_neigh; dim++) {

      // swap bonds with other proc
      // no MPI calls except SendRecv if nsend/nrecv = 0

      MPI_Sendrecv(&nsend,1,MPI_INT,sendproc[dim],me,
                   &nrecv,1,MPI_INT,recvproc[dim],recvproc[dim],world,MPI_STATUS_IGNORE);
      if (nrecv*size_bond > maxrecv) grow_recv(nrecv*size_bond);
      if (nrecv) MPI_Irecv(buf_recv,nrecv*size_bond,MPI_DOUBLE,
                           recvproc[dim],recvproc[dim],world,&request);
      if (nsend) MPI_Send(buf_send,nsend*size_bond,MPI_DOUBLE,sendproc[dim],me,world);
      ...
      for (i = 0; i < nrecv; i++){
        ...
        for (l = 0; l < 2; l++)
          bond_tags[nbond_tot][l] = (tagint) ubuf(buf_recv[m++]).i;
        bond_mol[nbond_tot] = (tagint) ubuf(buf_recv[m++]).i;
        bond_type[nbond_tot] = (int) ubuf(buf_recv[m++]).i;
        nbond_tot++;
      }
    }
    ...
    for (dim = 0; dim < 3; dim++) {
      ...
      for (ineed = 0; ineed < twoneed; ineed++) {
        ...
        if (ind){
          ...
          // swap bonds with other proc
          // put incoming ghosts at end of my bond arrays
          ...
          if (sendproc[iswap] != me) {
            ...
            if (nrecv*size_bond > maxrecv) grow_recv(nrecv*size_bond);
            if (nrecv) MPI_Irecv(buf_recv,nrecv*size_bond,MPI_DOUBLE,recvproc[iswap],0,world,&request);
            if (nsend) MPI_Send(buf_send,nsend*size_bond,MPI_DOUBLE,sendproc[iswap],0,world);
            if (nrecv) MPI_Wait(&request,MPI_STATUS_IGNORE);
          } else {
            nsend = 0;
            nrecv = 0;
  ...
  if (nbond_tot){
    ...
    if (tags_tot > max_tags_tot){
      ...
      memory->destroy(tags_part);
      memory->create(tags_part,max_tags_tot,"fix_force_bound_2d:tags_part");
    }
    ...
    if (max_bond > max_bond_part){
      max_bond_part = max_bond;
      memory->destroy(part_bond);
      memory->create(part_bond,max_npart_tot,max_bond_part,"fix_force_bound_2d:part_bond");
    }
  ...
  fprintf(out_stat,"nbond_tot=%d, max_bond_part=%d  \n", nbond_tot, max_bond_part); 
  fprintf(out_stat,"bonds  \n"); 
  for (i = 0; i < nbond_tot; i++)
    fprintf(out_stat,"%d    %ld %ld    %d %d   %d %d \n", i, bond_tags[i][0], bond_tags[i][1], bond_part[i][0], bond_part[i][1], bond_mol[i], bond_type[i]);
...
void FixForceBound2D::grow_bond_lists()
{
  max_nbond_tot += BUFEXTRA;
  memory->grow(bond_mol,max_nbond_tot,"fix_force_bound_2d:bond_mol");
  memory->grow(bond_type,max_nbond_tot,"fix_force_bound_2d:bond_type");
  memory->destroy(bond_part);
  memory->create(bond_part,max_nbond_tot,2,"fix_force_bound_2d:bond_part");
}
...
void FixForceBound2D::grow_part_lists()
{
  max_npart_tot = npart_tot + NUM1;
  memory->destroy(part_degree);
  memory->create(part_degree,max_npart_tot,"fix_force_bound_2d:part_degree");
  memory->destroy(part_tags);
  memory->create(part_tags,max_npart_tot,"fix_force_bound_2d:part_tags");
  memory->destroy(part_bond);
  memory->create(part_bond,max_npart_tot,max_bond_part,"fix_force_bound_2d:part_bond");
  memory->destroy(part_part_loc);
  memory->create(part_part_loc,max_npart_tot,"fix_force_bound_2d:part_part_loc");
        ...
        for (kk = 0; kk < num_bin_stensil[bin]; ++kk){
	      ...
	      if (rr < d_cut_sq)
                for (n = 0; n < part_degree[j]; n++){
                  k = part_bond[j][n];
                   if (part_part_loc[bond_part[k][0]] > -1 &&  part_part_loc[bond_part[k][1]] > -1) {
