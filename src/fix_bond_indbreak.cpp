...
FixBondIndBreak::FixBondIndBreak(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  partner(NULL), finalpartner(NULL), distsq(NULL), probability(NULL), broken(NULL), copy(NULL), random(NULL)
{
  if(atom->individual<1) error->all(FLERR,"Individual must be >0 for fix bond/indbreak command");
  if (narg < 6) error->all(FLERR,"Illegal fix bond/indbreak command");

  MPI_Comm_rank(world,&me);
  MPI_Comm_size(world,&nprocs);

  nevery = force->inumeric(FLERR,arg[3]);
  if (nevery <= 0) error->all(FLERR,"Illegal fix bond/indbreak command");
  ...
  breakdr = force->numeric(FLERR,arg[5]);

  if (btype < 1 || btype > atom->nbondtypes)
    error->all(FLERR,"Invalid bond type in fix bond/indbreak command");
  if (breakdr < 0.0) error->all(FLERR,"Illegal fix bond/indbreak command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"prob") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix bond/indbreak command");
      breakdr2 = force->numeric(FLERR,arg[iarg+1]);
      fraction = force->numeric(FLERR,arg[iarg+2]);
      seed = force->inumeric(FLERR,arg[iarg+3]);
      if(breakdr2 < breakdr)
        error->all(FLERR,"Illegal fix bond/indbreak command");
      if (fraction < 0.0 || fraction > 1.0)
        error->all(FLERR,"Illegal fix bond/indbreak command");
      if (seed <= 0) error->all(FLERR,"Illegal fix bond/indbreak command");
      iarg += 4;
    } else error->all(FLERR,"Illegal fix bond/indbreak command");
  }

  // error check

  if (atom->molecular != 1)
    error->all(FLERR,"Cannot use fix bond/indbreak with non-molecular systems");
...
FixBondIndBreak::~FixBondIndBreak()
{
  delete random;
...
void FixBondIndBreak::init()
{
  ...
  if (force->improper) {
    if (force->improper_match("class2") || force->improper_match("ring"))
      error->all(FLERR,"Cannot yet use fix bond/indbreak with this "
                 "improper style");
  }
...
void FixBondIndBreak::post_integrate()
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->destroy(probability);
    nmax = atom->nmax;
    memory->create(partner,nmax,"bond/indbreak:partner");
    memory->create(finalpartner,nmax,"bond/indbreak:finalpartner");
    memory->create(distsq,nmax,"bond/indbreak:distsq");
    memory->create(probability,nmax,"bond/indbreak:probability");
  }
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    breakr = breakdr + bondlist_length[n];
    breakrsq = breakr*breakr;
    if (rsq <= breakrsq) continue;
    ext = sqrt(rsq) - bondlist_length[n];
    if (ext > distsq[i1]) {
      partner[i1] = tag[i2];
      distsq[i1] = ext; //distsq stores the extension length
    }
    if (ext > distsq[i2]) {
      partner[i2] = tag[i1];
      distsq[i2] = ext;
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if (fraction < 1.0) {
        ...
        // }
        if (distsq[i]>=breakdr2) // Note that here distsq stores the extension length of bonds rather than the length of the bonds
          frac = fraction;
        else if(distsq[i]<breakdr2 && distsq[i]>=breakdr)
          frac = (distsq[i]-breakdr)/(breakdr2-breakdr)*fraction;
        else {
          printf("bond length is smaller than the break threshold\n");
          //printf("distsq[%d]=%f\n",i,distsq[i]);
          error->all(FLERR,"bond length is smaller than the break threshold");
        }

        if (tag[i] < tag[j]) {
          if (probability[i] >= frac) continue;
        } else {
          if (probability[j] >= frac) continue;
        }
    }
  ...
  for (i = 0; i < nall; i++) {
    ...
    if (j < 0 || tag[i] < tag[j]) {
      if (nbreak == maxbreak) {
        maxbreak += DELTA;
        memory->grow(broken,maxbreak,2,"bond/indbreak:broken");
      }
...
void FixBondIndBreak::check_ghosts()
{
  int i,j,n;
  tagint *slist;
  ...
  if (flagall)
    error->all(FLERR,"Fix bond/indbreak needs ghost atoms from further away");
...
void FixBondIndBreak::update_topology()
{
...
void FixBondIndBreak::rebuild_special_one(int m)
{
...
void FixBondIndBreak::break_angles(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixBondIndBreak::break_dihedrals(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixBondIndBreak::break_impropers(int m, tagint id1, tagint id2)
{
  int j,found;
...
int FixBondIndBreak::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
...
void FixBondIndBreak::post_integrate_respa(int ilevel, int /*iloop*/)
{
...
int FixBondIndBreak::pack_forward_comm(int n, int *list, double *buf,
...
void FixBondIndBreak::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixBondIndBreak::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixBondIndBreak::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixBondIndBreak::print_copy(const char *str, tagint m,
...
double FixBondIndBreak::compute_vector(int n)
{
...
double FixBondIndBreak::memory_usage()
{
  int nmax = atom->nmax;
