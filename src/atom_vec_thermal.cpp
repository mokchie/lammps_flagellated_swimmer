...
AtomVecThermal::AtomVecThermal(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecThermal::grow(int n)
{
  if (n == 0) grow_nmax();
  else nmax = n;
  atom->nmax = nmax;
  ...
  pte0 = memory->grow(atom->pte0,nmax,"atom:pte0");
  pte1 = memory->grow(atom->pte1,nmax,"atom:pte1");
...
void AtomVecThermal::grow_reset()
{
...
void AtomVecThermal::copy(int i, int j, int delflag)
{
  int k;
...
void AtomVecThermal::force_clear(int n, size_t nbytes)
{
  memset(&de[n],0,nbytes);
  memset(&pte1[n],0,nbytes);
}

/* ---------------------------------------------------------------------- */

int AtomVecThermal::pack_comm(int n, int *list, double *buf,
...
int AtomVecThermal::pack_comm_vel(int n, int *list, double *buf,
                                    int pbc_flag, int *pbc)
{
  int i,j,m;
...
void AtomVecThermal::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecThermal::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecThermal::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecThermal::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecThermal::pack_border(int n, int *list, double *buf,
                                  int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecThermal::pack_border_vel(int n, int *list, double *buf,
                                      int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecThermal::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecThermal::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecThermal::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecThermal::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecThermal::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecThermal::unpack_exchange(double *buf)
{
  int k;
...
int AtomVecThermal::size_restart()
{
  int i;
  ...
  if (individual){
    for (i = 0; i < nlocal; i++)
      n += 17 + 3*num_bond[i] + 5*num_angle[i] +
...
int AtomVecThermal::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecThermal::unpack_restart(double *buf)
{
  int k;
...
void AtomVecThermal::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecThermal::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecThermal::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecThermal::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecThermal::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecThermal::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecThermal::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecThermal::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("pte0")) bytes += memory->usage(pte0,nmax);
  if (atom->memcheck("pte1")) bytes += memory->usage(pte1,nmax);
