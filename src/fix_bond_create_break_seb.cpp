...
FixBondCreateBreakSeb::FixBondCreateBreakSeb(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 10) error->all(FLERR,"Illegal fix bond/create/break command");
  ...
  double cutoff_on = force->numeric(FLERR,arg[6]);
  double cutoff_off = force->numeric(FLERR,arg[7]);
  ...
  if (cutoff_on < 0.0) error->all(FLERR,"Illegal fix bond/create/break command");
  if (cutoff_off < 0.0) error->all(FLERR,"Illegal fix bond/create/break command");
  if (btype < 1 || btype > atom->nbondtypes)
    error->all(FLERR,"Invalid bond type in fix bond/create/break command");

  cutsq_on = cutoff_on*cutoff_on;
  cutsq_off = cutoff_off*cutoff_off;
  ...
  k0_off_catch = k0_off_slip = delta_catch = delta_slip = 0.0;
  temp = 1.0;
  const_on_rate = 0;

  if (strcmp(arg[9],"simple") == 0) type_flag = 0;
  else if (strcmp(arg[9],"dembo") == 0) {
    type_flag = 1;
    step = 6;
    if (narg < 9+step) error->all(FLERR,"Illegal fix bond/create/break command");
    spring = force->numeric(FLERR,arg[10]);
    k0_on = force->numeric(FLERR,arg[11]);
    k0_off = force->numeric(FLERR,arg[12]);
    sigma_on = force->numeric(FLERR,arg[13]);
    sigma_off = force->numeric(FLERR,arg[14]);
    temp = force->numeric(FLERR,arg[15]);
  } else if (strcmp(arg[9],"bell") == 0) {
    type_flag = 2;
    step = 7;
    if (narg < 9+step) error->all(FLERR,"Illegal fix bond/create/break command");
    spring = force->numeric(FLERR,arg[10]);
    k0_on = force->numeric(FLERR,arg[11]);
    k0_off = force->numeric(FLERR,arg[12]);
    sigma = force->numeric(FLERR,arg[13]);
    delta = force->numeric(FLERR,arg[14]);
    temp = force->numeric(FLERR,arg[15]);
    const_on_rate = force->numeric(FLERR,arg[16]);

  } else if (strcmp(arg[9],"catch/slip") == 0) {
    type_flag = 3;
    step = 8;
    if (narg < 9+step) error->all(FLERR,"Illegal fix bond/create/break command");
    spring = force->numeric(FLERR,arg[10]);
    k0_on = force->numeric(FLERR,arg[11]);
    k0_off_catch = force->numeric(FLERR,arg[12]);
    k0_off_slip = force->numeric(FLERR, arg[13]);
    sigma = force->numeric(FLERR,arg[14]);
    delta_catch = force->numeric(FLERR,arg[15]);
    delta_slip = force->numeric(FLERR, arg[16]);
    temp = force->numeric(FLERR,arg[17]);
...
//       iarg += 1;
      ...
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix bond/create/break command");
      interface_bonds = arg[iarg + 1];
      set_template_bonds = 0;
      iarg += 2;
...
//   comm_forward = MAX(2,2+atom->maxspecial); old
  ...
  createcount_try = 0;
  createcounttotal_try = 0;

  breakcount = 0;
  breakcounttotal = 0;

  breakcount_try = 0;
  breakcounttotal_try = 0;
...
FixBondCreateBreakSeb::~FixBondCreateBreakSeb()
{
...
int FixBondCreateBreakSeb::setmask()
{
  int mask = 0;
  mask |= POST_INTEGRATE;
...
void FixBondCreateBreakSeb::init()
{
  ...
  if (force->pair == NULL || sqrt(cutsq_on) > (sqrt(force->pair->cutsq[iatomtype][jatomtype]) + neighbor->skin) || sqrt(cutsq_off) > (sqrt(force->pair->cutsq[iatomtype][jatomtype]) + neighbor->skin)) {
    if (comm->me == 0) {
      std::cout << sqrt(cutsq_on) << "\t" << (sqrt(force->pair->cutsq[iatomtype][jatomtype]) + neighbor->skin) <<
      std::endl;
    }
...
void FixBondCreateBreakSeb::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixBondCreateBreakSeb::setup(int vflag)
{
  int i,j,m;
  ...
  if (countflag and set_template_bonds) return;

  atom->bond_per_atom = 20;
  ...
  std::ifstream template_bonds (interface_bonds.c_str());
  int b1, b2;
  while (template_bonds >> b1 >> b2){
    if (b2 < b1){
      int copy = b1;
      b1 = b2;
      b2 = copy;
    ...
    for (i = 0; i < nlocal; i++){
      if (tag[i] == b1){
        ...
        bond_atom[i][num_bond[i]] = b2;
        num_bond[i]++;
      }
    }
  }
  next_reneighbor = update->ntimestep;
  set_template_bonds = 1;
...
void FixBondCreateBreakSeb::post_integrate()
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(probability_b, nmax,"bond/create/break:probability_b");
    memory->create(probability_c, nmax,"bond/create/break:probability_c");
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if (rsq <= cutsq_off) continue;

    if (rsq > distsq_b[i1] && rsq > distsq_b[i2]) {
      partner_b[i1] = tag[i2];
      distsq_b[i1] = rsq;
      partner_b[i2] = -tag[i1];
      distsq_b[i2] = rsq;
  ...
  //std::ofstream data;
  //data.open("test.dat", std::ofstream::out | std::ofstream::app);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq >= cutsq_on) continue;

      if (rsq < distsq_c[i] && rsq < distsq_c[j]) {
        partner_c[i] = tag[j];
        distsq_c[i] = rsq;
        partner_c[j] = -tag[i];
        distsq_c[j] = rsq;
  ...
  if (type_flag == 0) {
    ...
    for (i = 0; i < nlocal; i++){
      if (partner_b[i]) {
        vec_fraction_b[i] = catch_slip_break(sqrt(distsq_b[i]));
        probability_b[i] = random->uniform();
      }
      if (partner_c[i]) {
        vec_fraction_c[i] = catch_slip_create(sqrt(distsq_c[i]));
        probability_c[i] = random->uniform();
      }
    }
    commflag = 3;
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if(!cont_flag_b) {
      jb = atom->map((tagint) abs(partner_b[i]));
      if (abs(partner_b[jb]) != tag[i]) cont_flag_b = 1;
    }

    if(!cont_flag_c) {
      jc = atom->map((tagint) abs(partner_c[i]));
      if (abs(partner_c[jc]) != tag[i]) cont_flag_c = 1;
    }
  ...
  MPI_Allreduce(&nbreak_try,&breakcount_try,1,MPI_INT,MPI_SUM,world);
  breakcounttotal_try += breakcount_try;
  MPI_Allreduce(&average_koff, &koff, 1, MPI_DOUBLE, MPI_SUM, world);
  koff_total += koff;
  MPI_Allreduce(&ncreate,&createcount,1,MPI_INT,MPI_SUM,world);
  createcounttotal += createcount;
  atom->nbonds += createcount;
  MPI_Allreduce(&ncreate_try,&createcount_try,1,MPI_INT,MPI_SUM,world);
  createcounttotal_try += createcount_try;
  MPI_Allreduce(&average_kon, &kon, 1, MPI_DOUBLE, MPI_SUM, world);
  kon_total += kon;
...
double FixBondCreateBreakSeb::dembo_create(double dist)
{
  double k_on;
  double probability;
...
double FixBondCreateBreakSeb::dembo_break(int i, int j, double dist)
{
...
double FixBondCreateBreakSeb::bell_create(double dist)
{
...
double FixBondCreateBreakSeb::bell_break(int i, int j, double dist)
{
...
double FixBondCreateBreakSeb::catch_slip_create(double dist)
{

  double k_on;
  double probability;

  k_on = k0_on * exp(-0.5 * sigma * fabs(dist - spring) * fabs(dist - spring) / temp);
...
double FixBondCreateBreakSeb::catch_slip_break(double dist){

  double k_off;
  double probability;


  k_off = k0_off_catch*exp(sigma* delta_catch * fabs( dist - spring )/ temp ) +
          k0_off_slip*exp(sigma* delta_slip * fabs( dist - spring) /temp);
  probability = 1.0 - exp(-k_off*dt);
...
void FixBondCreateBreakSeb::check_ghosts()
{
  int i,j,n;
  tagint *slist;
...
void FixBondCreateBreakSeb::post_integrate_respa(int ilevel, int iloop)
{
...
int FixBondCreateBreakSeb::pack_forward_comm(int n, int *list, double *buf,
...
void FixBondCreateBreakSeb::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixBondCreateBreakSeb::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixBondCreateBreakSeb::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixBondCreateBreakSeb::grow_arrays(int nmax)
{
...
void FixBondCreateBreakSeb::copy_arrays(int i, int j, int delflag)
{
...
int FixBondCreateBreakSeb::pack_exchange(int i, double *buf)
{
  buf[0] = bondcount[i];
  return 1;
}
...
int FixBondCreateBreakSeb::unpack_exchange(int nlocal, double *buf)
{
...
double FixBondCreateBreakSeb::compute_vector(int n)
{
  //printf("In compute_vector\n");
  switch (n){
    case 0: return (double) (createcounttotal - breakcounttotal);
    case 1: return (double) breakcounttotal;
    case 2: return (double) createcounttotal;
    case 3: return (double) breakcounttotal_try;
    case 4: return (double) createcounttotal_try;
    case 5: return koff_total;
    case 6: return kon_total;
  }
}
...
double FixBondCreateBreakSeb::memory_usage()
{
  int nmax = atom->nmax;
...
void FixBondCreateBreakSeb::print_bb()
{
...
void FixBondCreateBreakSeb::print_copy(const char *str, tagint m,
