...
FixStyle(squirmer,FixSquirmer)

#else

#ifndef LMP_FIX_SQUIRMER_H
#define LMP_FIX_SQUIRMER_H

#include "fix.h"

namespace LAMMPS_NS {

class FixSquirmer : public Fix {
 public:
  FixSquirmer(class LAMMPS *, int, char **);
  virtual ~FixSquirmer();
  virtual int setmask();
  virtual void init();
 ...
 protected:
  ...
  double Rs;                // radius of the squirmers
  double B1;                // source dipole strength
  double beta;              // squirmer parameter, beta=B2/B1, B2 is the force dipole strength
  ...
  double **fcm_active;     // force on squirmer due to the slip velocity
  double **inertia;         // 3 principal components of inertia of each
  double **ex_space,**ey_space,**ez_space;

  double **orientation;    // swimming direction of the squirmer
  ...
  double **torque_active;  // torque on squirmer due to the slip velocity
  ...
  int dpd_group_bit;      // mask for dpd group
  char *id_dpd;           // group name to dpd
