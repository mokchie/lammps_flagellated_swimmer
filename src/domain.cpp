...
void Domain::set_local_box()
{
  ...
  if (comm->layout != Comm::LAYOUT_TILED) {
    ...
    if (comm->user_part){
      if (comm->processed_data_ind){
        for (i = 0; i < 3; i++){
          sublo[i] = boxlo[i] + comm->nbp_min[i]*comm->bin_size[i];
          subhi[i] = boxlo[i] + (comm->nbp_max[i] - 2*comm->bin_ext[i] + 1)*comm->bin_size[i];
        }
      } else {
        for (i = 0; i < 3; i++){
          sublo[i] = boxlo[i] + comm->nbp_min[i]*comm->bin_size[i];
          subhi[i] = boxlo[i] + (comm->nbp_max[i] + 1)*comm->bin_size[i];
        }
      }
    } else {
