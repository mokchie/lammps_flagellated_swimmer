...
BondArea_WLC_POW_VISC::BondArea_WLC_POW_VISC(LAMMPS *lmp) : Bond(lmp) 
{
  random = NULL;
  mtrand = NULL;
  dath = datt = NULL;
  n_mol_limit = MOLINC;
  memory->create(dath,2*n_mol_limit,"bond_area_wlc_pow_visc:dath");
  memory->create(datt,2*n_mol_limit,"bond_area_wlc_pow_visc:datt");  
}
...
BondArea_WLC_POW_VISC::~BondArea_WLC_POW_VISC()
{
  if (allocated) {
...
void BondArea_WLC_POW_VISC::compute(int eflag, int vflag)
{
  int i1,i2,n,type,factor,k,l;
  tagint m, m1;
  double rsq,r,rfactor,l0,aa,rlogarg;
  double dvx, dvy, dvz;
  double coefa,vv,rr,lmax,kph,Y0h,lambda;
  double xx1[3],xx2[3];
  ...
  if (n_mol_max > n_mol_limit){
    ...
    memory->grow(dath,2*n_mol_limit,"bond_area_wlc_pow_visc:dath");
    memory->grow(datt,2*n_mol_limit,"bond_area_wlc_pow_visc:datt");
  }
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    lmax = l0*r0[type];
    rr = 1.0/r0[type];
    kph = pow(l0,pp[type])*(0.25/(1.0-rr)/(1.0-rr)-0.25+rr);
    Y0h = 0.5*rr/pow(1.0-rr,3) + rr + kph*pp[type]/pow(l0,pp[type]);
    lambda = Y0[type]/Y0h;
    kph *= lambda;
    rr = r/lmax;
    rlogarg = pow(r,pp[type]+1.0);
    ...
    fbond = -(lambda*(0.25/(1.0-rr)/(1.0-rr)-0.25+rr) + kl[type]*(datt[m] - circum[type]))/r + kph/rlogarg + (sigc[type]*wrr[3] - gamc[type]*vv)/r;

    if (eflag){
      ebond = 0.25*lambda*lmax*(3.0*rr*rr-2.0*rr*rr*rr)/(1.0-rr);
      if (pp[type] == 1.0)
        ebond -= kph*log(r);
      else
        ebond += kph/(pp[type]-1.0)/pow(r,pp[type]-1.0);
    }
...
void BondArea_WLC_POW_VISC::allocate()
{
  allocated = 1;
  ...
  if(individual == 0)
    error->all(FLERR,"Individual has wrong value or is not set! Using bond area/wlc/pow/visc is only possible with individual = 1.");

  Y0 = (double *) memory->smalloc((n+1)*sizeof(double),"bond:Y0");
  r0 = (double *) memory->smalloc((n+1)*sizeof(double),"bond:r0");
  pp = (double *) memory->smalloc((n+1)*sizeof(double),"bond:pp");
...
void BondArea_WLC_POW_VISC::coeff(int narg, char **arg)
{
  if (narg != 11) error->all(FLERR,"Incorrect args in bond_coeff command");
  if (!allocated) allocate();

  int ilo,ihi;
  force->bounds(FLERR,arg[0],atom->nbondtypes,ilo,ihi);

  double Y0_one = force->numeric(FLERR,arg[1]);
  double r0_one = force->numeric(FLERR,arg[2]);
  double pp_one = force->numeric(FLERR,arg[3]);
  double kl_one = force->numeric(FLERR,arg[4]);
  double circum_one = force->numeric(FLERR,arg[5]);
  double ka_one = force->numeric(FLERR,arg[6]);
  double area_one = force->numeric(FLERR,arg[7]);
  double temp_one = force->numeric(FLERR,arg[8]);
  double gamc_one = force->numeric(FLERR,arg[9]);
  double gamt_one = force->numeric(FLERR,arg[10]);
...
void BondArea_WLC_POW_VISC::init_style()
{ 
...
double BondArea_WLC_POW_VISC::equilibrium_distance(int i)
{
  return r0[i];
}
...
void BondArea_WLC_POW_VISC::write_restart(FILE *fp)
{
  fwrite(&Y0[1],sizeof(double),atom->nbondtypes,fp);
  fwrite(&r0[1],sizeof(double),atom->nbondtypes,fp);
  fwrite(&pp[1],sizeof(double),atom->nbondtypes,fp);
...
void BondArea_WLC_POW_VISC::read_restart(FILE *fp)
{
  allocate();

  if (comm->me == 0) {
    fread(&Y0[1],sizeof(double),atom->nbondtypes,fp);
    fread(&r0[1],sizeof(double),atom->nbondtypes,fp);
    fread(&pp[1],sizeof(double),atom->nbondtypes,fp);
  ...
  MPI_Bcast(&Y0[1],atom->nbondtypes,MPI_DOUBLE,0,world);
  MPI_Bcast(&r0[1],atom->nbondtypes,MPI_DOUBLE,0,world);
  MPI_Bcast(&pp[1],atom->nbondtypes,MPI_DOUBLE,0,world);
...
void BondArea_WLC_POW_VISC::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nbondtypes; i++)
    fprintf(fp,"%d %g %g %g %g %g %g %g %g %g %g\n",i,Y0[i],r0[i],pp[i],kl[i],circum[i],ka[i],area[i],temp[i],gamc[i],gamt[i]);
}

/* ---------------------------------------------------------------------- */

double BondArea_WLC_POW_VISC::single(int type, double rsq, int i, int j,
                        double &fforce)
{
  char warning[128];
...
void BondArea_WLC_POW_VISC::generate_wrr()
{
  int i;
  double ww[3][3];
