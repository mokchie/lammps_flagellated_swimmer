...
enum{VERSION,SMALLINT,TAGINT,BIGINT,
     ...
     INDIVIDUAL,ATOM_STYLE,NATOMS,NTYPES,
     ...
     MASS,MOMENT,PAIR,BOND,ANGLE,DIHEDRAL,IMPROPER,
...
void ReadRestart::type_arrays()
{
  ...
  while (flag >= 0) {
    ...
    if (flag == MASS) {
      ...
      read_int(); 
      double *moment = new double[atom->ntypes+1];
      read_double_vec(atom->ntypes,&moment[1]);
      atom->set_moment(moment);
      delete [] moment;
