...
#ifndef LMP_FIX_WALL_FORCE_H
#define LMP_FIX_WALL_FORCE_H

#include "fix.h"

namespace LAMMPS_NS {

class FixWallForce : public Fix {
 public:
  int nwall;
  int wallwhich[6];
  double coord0[6];
  ...
  FixWallForce(class LAMMPS *, int, char **);
  virtual ~FixWallForce();
  int setmask();
  virtual void init();
  void setup(int);
  void min_setup(int);
  ...
  void wall_integration(int, double);


  virtual void precompute(int) = 0;
  virtual double wall_particle(int, int, double) = 0;

 protected:
  double epsilon[6],sigma[6],cutoff[6],force_push[6],friction[6];
  double coordinate[6], coordinate_old[6];
