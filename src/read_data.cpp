...
void ReadData::command(int narg, char **arg)
{
  ...
  while (1) {
    ...
    while (strlen(keyword)) {
      ...
      } else if (strcmp(keyword,"Moments") == 0) {
        if (firstpass) moment();
        else skip_lines(ntypes);
...
void ReadData::header(int firstpass)
{
  ...
  const char *section_keywords[NSECTIONS] =
    {"Atoms","Velocities","Ellipsoids","Lines","Triangles","Bodies",
     ...
     "Masses","Moments","Pair Coeffs","PairIJ Coeffs","Bond Coeffs","Angle Coeffs",
...
void ReadData::moment()
{
  ...
  for (int i = 0; i < ntypes; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    atom->set_moment(FLERR,buf,toffset);
    buf = next + 1;
  }
  delete [] original;
}
