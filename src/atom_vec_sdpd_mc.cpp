...
AtomVecSDPDMC::AtomVecSDPDMC(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecSDPDMC::grow(int n)
{
  ...
  grain = memory->grow(atom->grain,nmax,"atom:grain");
...
void AtomVecSDPDMC::copy(int i, int j, int delflag)
{
  int k;
...
int AtomVecSDPDMC::pack_comm(int n, int *list, double *buf,
...
int AtomVecSDPDMC::pack_comm_vel(int n, int *list, double *buf,
                                    int pbc_flag, int *pbc)
{
  int i,j,m;
...
void AtomVecSDPDMC::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDMC::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
  ...
  for (i = first; i < last; i++) {
    ...
    omega[i][1] = buf[m++];
    omega[i][2] = buf[m++];
    grain[i] = (int) ubuf(buf[m++]).i;
  }
}

/* ---------------------------------------------------------------------- */

int AtomVecSDPDMC::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPDMC::pack_reverse_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDMC::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPDMC::unpack_reverse_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPDMC::pack_border(int n, int *list, double *buf,
                                  int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecSDPDMC::pack_border_vel(int n, int *list, double *buf,
                                      int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecSDPDMC::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecSDPDMC::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDMC::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPDMC::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPDMC::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecSDPDMC::unpack_exchange(double *buf)
{
  int k;
...
int AtomVecSDPDMC::size_restart()
{
  int i;
  ...
  if (individual){
    for (i = 0; i < nlocal; i++)
      n += 21 + 3*num_bond[i] + na*num_angle[i] +
        6*num_dihedral[i] + 5*num_improper[i];
  }
  else{
    for (i = 0; i < nlocal; i++)
      n += 21 + 2*num_bond[i] + 4*num_angle[i] +
...
int AtomVecSDPDMC::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecSDPDMC::unpack_restart(double *buf)
{
  int k;
...
void AtomVecSDPDMC::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecSDPDMC::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecSDPDMC::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecSDPDMC::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecSDPDMC::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecSDPDMC::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecSDPDMC::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecSDPDMC::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("grain")) bytes += memory->usage(grain,nmax);
