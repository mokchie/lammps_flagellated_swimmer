...
FixSolidBound::FixSolidBound(LAMMPS *lmp, int narg, char **arg) : Fix(lmp,narg, arg)
{
 ...
 //read in plane.dat file except shapes
  ...
  if (me)
    memory->create(rdata,l,"fix_solid_bound:rdata");
  ...
  if (ind_press){       
    ...
    memory->create(f_press,n_press+1,"fix_solid_bound:f_press");
    for (j=0; j<n_press; j++)
      f_press[j] = rdata[l++];
    f_press[n_press] = 0.0;
  }
  ...
  memory->create(aa,num_shapes_tot,8,"fix_solid_bound:aa");
...
{
  int i,j,k,kk,ind,cond,i_x[NUM2],cc,bounce,m,cplane,ccount;
  double dl[3], t_x[NUM2], dd[3],xp[3],xh[3],vh[3],vv[3],norm[3];
...
void FixSolidBound::recalc_force()
{
  ...
  memory->create(tmp,numt,"fix_solid_bound:tmp");
  memory->create(ntmp,numt,"fix_solid_bound:ntmp");
...
void FixSolidBound::write_restart(FILE *fp)
{
  ...
  if (numt_loc)
    ...
    offset = 1;
  memory->create(tmp,offset,"fix_solid_bound:tmp");
  ...
  if (l != numt_loc)
    error->warning(FLERR,"Something might be wrong: numt_loc and l are not equal in FixSolidBound::write_restart!");

  memory->create(tmph,nel_tot,"fix_solid_bound:tmph");
  if (comm->nprocs > 1){
  ...
  if (me == 0){
    memory->create(list,numt_s*3+1,"fix_solid_bound:list");
    kk = 0;
    for (i=0; i<num_shapes_tot; i++){
      l = 0;
      ind_sh = 0;
    ...
    if (kk != numt_s*3)
      error->warning(FLERR,"Something might be wrong: kk and 3*numt_s are not equal in FixSolidBound::write_restart!");
...
void FixSolidBound::restart(char *buf)
{
  int i,j,k,l,m;
