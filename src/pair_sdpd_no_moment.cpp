...
PairSDPDNoMoment::PairSDPDNoMoment(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
PairSDPDNoMoment::~PairSDPDNoMoment()
{
  if (allocated) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            gam = r5_3*eta[itype][jtype]*wd/rho[i]/rho[j];
            ...
            fnc[0] = factor_dpd*(sig*wrr[0] - delvx*gam);
            fnc[1] = factor_dpd*(sig*wrr[1] - delvy*gam);
	    fnc[2] = factor_dpd*(sig*wrr[2] - delvz*gam);
            //fnc[0] = factor_dpd*(sig*wrr[0]*rinv - delvx*gam); // use with generate_wrr();
            //fnc[1] = factor_dpd*(sig*wrr[1]*rinv - delvy*gam); // use with generate_wrr();
            //fnc[2] = factor_dpd*(sig*wrr[2]*rinv - delvz*gam); // use with generate_wrr();
...
void PairSDPDNoMoment::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairSDPDNoMoment::settings(int narg, char **arg)
{
  ...
  if (grph == -1) error->all(FLERR,"Could not find group ID in sdpd/no/moment");
  groupbit = group->bitmask[grph];
  cut_global = force->numeric(FLERR,arg[5]);
  seed = force->inumeric(FLERR,arg[6]);
  grph = group->find(arg[7]);
  if (grph == -1) error->all(FLERR,"Could not find group_rho ID in sdpd/no/moment");
...
void PairSDPDNoMoment::coeff(int narg, char **arg)
{
...
void PairSDPDNoMoment::init_style()
{
...
double PairSDPDNoMoment::init_one(int i, int j)
{
...
void PairSDPDNoMoment::write_restart(FILE *fp)
{
...
void PairSDPDNoMoment::read_restart(FILE *fp)
{
...
void PairSDPDNoMoment::write_restart_settings(FILE *fp)
{
...
void PairSDPDNoMoment::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDNoMoment::single(int i, int j, int itype, int jtype, double rsq,
...
void PairSDPDNoMoment::set_weight()
{
  int i,j,k,l;
  int n = atom->ntypes;
  double rr,al1,al2;
  ...
  if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDNoMoment::set_weight.");
  ...
  for (i = 1; i <= n; i++)
    for (j = i; j <= n; j++){
      ...
      if (k > nw_max) error->all(FLERR,"Error in PairSDPD::set_weight - k > nw_max");
...
void PairSDPDNoMoment::generate_wrr()
{
  int i;
  double ww[3][3];
...
int PairSDPDNoMoment::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPDNoMoment::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
int PairSDPDNoMoment::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
void PairSDPDNoMoment::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
