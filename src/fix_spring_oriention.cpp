...
#include "fix_spring_orientation.h"
#include "atom.h"
#include "comm.h"
#include "update.h"
#include "respa.h"
...
FixSpringOrientation::FixSpringOrientation(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 7) error->all(FLERR,"Illegal fix spring/orientation command");
  ...
  if(strcmp(arg[5],"xy")==0) plane = XY;
  else if (strcmp(arg[5],"xz")==0) plane = XZ;
  else if (strcmp(arg[5],"yz")==0) plane = YZ;
  else error->all(FLERR,"Spring/orientation: Wrong plane specified");
...
FixSpringOrientation::~FixSpringOrientation()
{
...
int FixSpringOrientation::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
  mask |= THERMO_ENERGY;
...
void FixSpringOrientation::init()
{
...
void FixSpringOrientation::setup(int vflag)
{
...
void FixSpringOrientation::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixSpringOrientation::post_force(int /*vflag*/)
{
  int n1,n2,n3;
...
void FixSpringOrientation::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixSpringOrientation::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixSpringOrientation::compute_scalar()
{
  return espring;
}
...
double FixSpringOrientation::compute_vector(int n)
{
  return ftotal[n];
}
