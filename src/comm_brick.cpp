...
void CommBrick::forward_comm(int /*dummy*/)
{
  ...
  for (int iswap = 0; iswap < nswap; iswap++) {
    ...
    // lees-edwards fix
    if (le && iswap == 2*maxneed[0]) lees_edwards(0);
...
void CommBrick::borders()
{
  ...
  for (dim = 0; dim < 3; dim++) {
    ...
    for (ineed = 0; ineed < twoneed; ineed++) {
      //printf("dim=%d,twoneed=%d,ineed=%d\n",dim,twoneed,ineed);
      ...
      //printf("the programe goes to here\n");
      smax = MAX(smax,nsend);
      rmax = MAX(rmax,nrecv);
      sendnum[iswap] = nsend;
      recvnum[iswap] = nrecv;
...
void CommBrick::lees_edwards(int n)
{
  int i,j;
  ...
  if (n && le_nmax < le_nall - nlocal){
    ...
    memory->grow(le_sh,le_nmax,"comm:le_sh");
  }
