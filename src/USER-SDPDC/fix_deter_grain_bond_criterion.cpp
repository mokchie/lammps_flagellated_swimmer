...
#include <cstring>
#include <cstdlib>
#include "fix_deter_grain_bond_criterion.h"
#include "atom.h"
#include "update.h"
#include "modify.h"
#include "domain.h"
...
FixDeterGrainBondCriterion::FixDeterGrainBondCriterion(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  xstr(NULL), ystr(NULL), zstr(NULL), idregion(NULL), sforce(NULL), totalnum_bond(NULL), totalnum_hardbond(NULL)
{
  if (narg < 8) error->all(FLERR,"Illegal fix deter/grain/bond/criterion command");
...
FixDeterGrainBondCriterion::~FixDeterGrainBondCriterion()
{
  if (copymode) return;
  ...
  memory->destroy(totalnum_hardbond);  
}

/* ---------------------------------------------------------------------- */

int FixDeterGrainBondCriterion::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
...
void FixDeterGrainBondCriterion::init()
{
  // check variables
...
void FixDeterGrainBondCriterion::setup(int vflag)
{
...
void FixDeterGrainBondCriterion::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixDeterGrainBondCriterion::post_force(int /*vflag*/)
{
  double **x = atom->x;
  double **f = atom->f;
  int *grain = atom->grain;
  if (grain==NULL) 
    error->all(FLERR,"Fix deter/grain/bond/criterion only work with sdpd/mc, where the array 'grain' is defined");  
  int *mask = atom->mask;
  ...
  if(atom->nmax > nmax){
    ...
    memory->create(totalnum_hardbond,nmax,"setforce/bond/criterion:totalnum_hardbond");    
  }
  for(int i = 0; i < nall; i++){
    totalnum_bond[i] = 0;
    totalnum_hardbond[i] = 0;
  }
  ...
  for (n=0; n<nbondlist; n++){
    ...
    if(!(mask[i2] & groupbit)) continue;
    if(type != btype) continue;
    if((!force->newton_bond && totalnum_bond[i2]>=2*bnc) || (force->newton_bond && totalnum_bond[i2]>=bnc))
      totalnum_hardbond[i1]++;
    if((!force->newton_bond && totalnum_bond[i1]>=2*bnc) || (force->newton_bond && totalnum_bond[i1]>=bnc))    
      totalnum_hardbond[i2]++;
  }
  // update region if necessary
  commflag = 2;
  comm->reverse_comm_fix(this);
...
void FixDeterGrainBondCriterion::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixDeterGrainBondCriterion::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixDeterGrainBondCriterion::compute_vector(int n)
{
...
double FixDeterGrainBondCriterion::memory_usage()
{
  double bytes = 0.0;
  int nmax = atom->nmax;
...
int FixDeterGrainBondCriterion::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  if(commflag==1){
    m = 0;
    ...
    last = first + n;
    for (i = first; i < last; i++) {
      buf[m++] = ubuf(totalnum_hardbond[i]).d;
    }
    return m;    
  }
}
void FixDeterGrainBondCriterion::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  if(commflag==1){
    m = 0;
    ...
    for (i = 0; i < n; i++) {
      j = list[i];
      totalnum_hardbond[j] += ubuf(buf[m++]).i;
    }    
  }
}
