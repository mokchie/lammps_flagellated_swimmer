...
PairSDPDC::PairSDPDC(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
void PairSDPDC::compute(int eflag, int vflag)
{
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            r = sqrt(rsq);
            if (r < EPSILON) continue;     // r can be 0.0 in SDPDC systems
            rinv = 1.0/r;
...
void PairSDPDC::settings(int narg, char **arg)
{
  ...
  if (grph == -1) error->all(FLERR,"Could not find group ID in sdpdc");
  groupbit = group->bitmask[grph];
  cut_global = force->numeric(FLERR,arg[5]);
  seed = force->inumeric(FLERR,arg[6]);
  grph = group->find(arg[7]);
  if (grph == -1) error->all(FLERR,"Could not find group_rho ID in sdpdc");
  ...
  if(narg<13){
    ...
    if (grph == -1) error->all(FLERR,"Could not find group_fines ID in sdpdc");
    groupbit_fines = group->bitmask[grph];
  }
...
void PairSDPDC::coeff(int narg, char **arg)
{
...
void PairSDPDC::init_style()
{
  if (comm->ghost_velocity == 0)
    error->all(FLERR,"Pair sdpdc requires ghost atoms store velocity");
...
double PairSDPDC::init_one(int i, int j)
{
...
void PairSDPDC::write_restart(FILE *fp)
{
...
void PairSDPDC::read_restart(FILE *fp)
{
...
void PairSDPDC::write_restart_settings(FILE *fp)
{
...
void PairSDPDC::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDC::single(int i, int j, int itype, int jtype, double rsq,
...
double PairSDPDC::single_vec(int i, int j, int itype, int jtype, double rsq,
...
//   nw_max = static_cast<int> (rr*dr_inv) + 2;
//   if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDC::set_weight.");
//   memory->create(weight1,n+1,n+1,nw_max,"pair:weight1");
//   memory->create(weight2,n+1,n+1,nw_max,"pair:weight2");
...
//     for (j = i; j <= n; j++){
//       k = static_cast<int> (cut[i][j]*dr_inv) + 1;
//       if (k > nw_max) error->all(FLERR,"Error in PairSDPDC::set_weight - k > nw_max");
...
int PairSDPDC::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
  double *solid_rho = atom->solid_rho;
...
void PairSDPDC::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double *solid_rho = atom->solid_rho;
...
int PairSDPDC::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double *solid_rho = atom->solid_rho;
...
void PairSDPDC::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
  double *solid_rho = atom->solid_rho;
