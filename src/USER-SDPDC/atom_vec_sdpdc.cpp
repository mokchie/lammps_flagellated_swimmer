...
AtomVecSDPDC::AtomVecSDPDC(LAMMPS *lmp) : AtomVec(lmp)
{
  if (individual<1) error->all(FLERR,"for atom style of sdpdc, individual must be larger than 0");
  molecular = 1;
  ...
  comm_f_only = 0;
  size_forward = 3;
  size_reverse = 6+individual;
  size_border = 8;
  size_velocity = 8+individual;
  size_data_atom = 6+individual;
  size_data_vel = 4;
  xcol_data = 4+individual;
...
void AtomVecSDPDC::grow(int n)
{
  ...
  fixfine = memory->grow(atom->fixfine,nmax,"atom:fixfine");
  fixed = memory->grow(atom->fixed,nmax,"atom:fixed");  
  conc = memory->grow(atom->conc,nmax,individual,"atom:conc");
  f_conc = memory->grow(atom->f_conc,nmax*comm->nthreads,individual,"atom:f_conc");
  rho = memory->grow(atom->rho,nmax,"atom:rho");
  solid_rho = memory->grow(atom->solid_rho,nmax,"atom:solid_rho");  
  fluid_rho = memory->grow(atom->fluid_rho,nmax,"atom:fluid_rho");
  ...
  if (individual){
    ...
    bond_length0 = memory->grow(atom->bond_length0,nmax,atom->bond_per_atom,"atom:bond_length0");
...
void AtomVecSDPDC::grow_reset()
{
  tag = atom->tag; type = atom->type;
  mask = atom->mask; image = atom->image;
  x = atom->x; v = atom->v; f = atom->f;
  omega = atom->omega; torque = atom->torque;
  conc = atom->conc; f_conc = atom->f_conc;
  fixfine = atom->fixfine;
  fixed = atom->fixed;
  rho = atom->rho;
  solid_rho = atom->solid_rho;
  fluid_rho = atom->fluid_rho;
  spin = atom->spin;
  ...
  if (individual){
    ...
    bond_length0 = atom->bond_length0;
...
void AtomVecSDPDC::copy(int i, int j, int delflag)
{
  int k;
  ...
  if (individual){
    for (k = 0; k < num_bond[j]; k++){
      ...
      bond_length0[j][k] = bond_length0[i][k];
    }
    for (k = 0; k < num_angle[j]; k++)
...
int AtomVecSDPDC::pack_comm(int n, int *list, double *buf,
...
int AtomVecSDPDC::pack_comm_vel(int n, int *list, double *buf,
                                    int pbc_flag, int *pbc)
{
  int i,j,m,k;
  ...
  if (pbc_flag == 0) {
    for (i = 0; i < n; i++) {
      ...
      buf[m++] = omega[j][1];
      buf[m++] = omega[j][2];
      buf[m++] = ubuf(fixfine[j]).d;
      buf[m++] = ubuf(fixed[j]).d;
      for (k=0; k<individual; k++){
        buf[m++] = conc[j][k];
      }
    }
...
void AtomVecSDPDC::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDC::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last,k;
  ...
  for (i = first; i < last; i++) {
    ...
    omega[i][1] = buf[m++];
    omega[i][2] = buf[m++];
    fixfine[i] = (int) ubuf(buf[m++]).i;
    fixed[i] = (int) ubuf(buf[m++]).i;
    for (k=0; k<individual; k++){
      conc[i][k] = buf[m++];
    }
  }
...
int AtomVecSDPDC::pack_reverse(int n, int first, double *buf)
{
  int i,m,last,k;
...
int AtomVecSDPDC::pack_reverse_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDC::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m,k;
...
int AtomVecSDPDC::unpack_reverse_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPDC::pack_border(int n, int *list, double *buf,
                                  int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecSDPDC::pack_border_vel(int n, int *list, double *buf,
                                      int pbc_flag, int *pbc)
{
  int i,j,m,k;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecSDPDC::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecSDPDC::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDC::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last,k;
...
int AtomVecSDPDC::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPDC::pack_exchange(int i, double *buf)
{
  int k;
  ...
  buf[m++] = omega[i][1];
  buf[m++] = omega[i][2];
  buf[m++] = ubuf(fixfine[i]).d;
  buf[m++] = ubuf(fixed[i]).d;
  for (k=0; k<individual; k++){
    buf[m++] = conc[i][k];
  }
...
int AtomVecSDPDC::unpack_exchange(double *buf)
{
  int k;
  ...
  fixfine[nlocal] = (int) ubuf(buf[m++]).i;
  fixed[nlocal] = (int) ubuf(buf[m++]).i;
  for (k=0; k<individual; k++){
    conc[nlocal][k] = buf[m++];
  }
...
int AtomVecSDPDC::size_restart()
{
  ...
  if (individual){
    for (i = 0; i < nlocal; i++)
      n += 22 + individual + 4*num_bond[i] + 5*num_angle[i] +
        6*num_dihedral[i] + 5*num_improper[i];
  }
  else{
    for (i = 0; i < nlocal; i++)
      n += 22 + individual + 2*num_bond[i] + 4*num_angle[i] +
...
int AtomVecSDPDC::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecSDPDC::unpack_restart(double *buf)
{
  int k;
...
void AtomVecSDPDC::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  int k;
  if (nlocal == nmax) grow(0);
...
void AtomVecSDPDC::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  int k;
  if (nlocal == nmax) grow(0);
...
int AtomVecSDPDC::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecSDPDC::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecSDPDC::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecSDPDC::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecSDPDC::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecSDPDC::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("conc")) bytes += memory->usage(conc,nmax,individual);
  if (atom->memcheck("f_conc")) bytes += memory->usage(f_conc,nmax*comm->nthreads,individual);
  if (atom->memcheck("rho")) bytes += memory->usage(rho,nmax); 
  if (atom->memcheck("solid_rho")) bytes += memory->usage(solid_rho,nmax);   
  if (atom->memcheck("fluid_rho")) bytes += memory->usage(fluid_rho,nmax);
  ...
  if (atom->memcheck("bond_length0"))
      bytes += memory->usage(bond_length0,nmax,atom->bond_per_atom);    
