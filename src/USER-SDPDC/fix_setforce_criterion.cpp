...
#include <cstring>
#include <cstdlib>
#include "fix_setforce_criterion.h"
#include "atom.h"
#include "update.h"
#include "modify.h"
#include "domain.h"
...
FixSetForceCriterion::FixSetForceCriterion(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  xstr(NULL), ystr(NULL), zstr(NULL), idregion(NULL), sforce(NULL)
{
  if (narg < 7) error->all(FLERR,"Illegal fix setforce/criterion command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix setforce/criterion command");
      iregion = domain->find_region(arg[iarg+1]);
      if (iregion == -1)
        error->all(FLERR,"Region ID for fix setforce/criterion does not exist");
      int n = strlen(arg[iarg+1]) + 1;
      idregion = new char[n];
      strcpy(idregion,arg[iarg+1]);
      iarg += 2;
    } else error->all(FLERR,"Illegal fix setforce/criterion command");
  }
...
FixSetForceCriterion::~FixSetForceCriterion()
{
  if (copymode) return;
...
int FixSetForceCriterion::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
...
void FixSetForceCriterion::init()
{
  // check variables
  ...
  int *fixed = atom->fixed;
  if (fixed==NULL) 
    error->all(FLERR,"Fix setforce/criterion only work with SDPDC, where the array 'fixed' is defined");
...
void FixSetForceCriterion::setup(int vflag)
{
...
void FixSetForceCriterion::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixSetForceCriterion::post_force(int /*vflag*/)
{
  double **x = atom->x;
  double **f = atom->f;
  int *mask = atom->mask;
...
void FixSetForceCriterion::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixSetForceCriterion::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixSetForceCriterion::compute_vector(int n)
{
...
double FixSetForceCriterion::memory_usage()
{
  double bytes = 0.0;
