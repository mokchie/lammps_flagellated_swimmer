...
FixSetVelInd::FixSetVelInd(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 9) error->all(FLERR,"Illegal fix setvel/ind command");
...
void FixSetVelInd::post_integrate()
{
  int i;
  double factor;
