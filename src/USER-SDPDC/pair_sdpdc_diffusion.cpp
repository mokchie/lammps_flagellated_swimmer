...
PairSDPDCDiffusion::PairSDPDCDiffusion(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
PairSDPDCDiffusion::~PairSDPDCDiffusion()
{
  if (allocated) {
...
void PairSDPDCDiffusion::compute(int eflag, int vflag)
{
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            r = sqrt(rsq);
            if (r < EPSILON) continue;     // r can be 0.0 in 
            ...
            if (update->ntimestep*update->dt>=t0){
              for (k = 0; k < atom->individual; k++){
		if (((mask[i] & groupbit_solid) && conc[i][k]<conc[j][k] && !(mask[j] & groupbit_solid)) || (!(mask[i] & groupbit_solid) && conc[i][k]>conc[j][k] && (mask[j] & groupbit_solid))) cij_d=0; //conpounds are not allowed to diffuse back to the solids
		else cij_d = conc[i][k]-conc[j][k];
		if (((mask[i] & groupbit_solid) && !(mask[j] & groupbit_solid)) || (!(mask[i] & groupbit_solid) && (mask[j] & groupbit_solid))) cij_c=0;
		else cij_c = conc[i][k]-conc[j][k];
                ...
                //   continue;                             
                //f_conc[i][k] += -wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Df[itype][jtype][k]*rho[i]+Df[jtype][itype][k]*rho[j]) - wd * (conc[i][k]-conc[j][k])/rho[i] * (v[i][0] * delx + v[i][1] * dely + v[i][2] * delz);
                f_conc[i][k] += - wd * cij_c/rho[i] * (v[i][0] * delx + v[i][1] * dely + v[i][2] * delz) -wd * cij_d/rho[i]/rho[j]*(Df[itype][jtype][k]*rho[i]+Df[jtype][itype][k]*rho[j]);

                if (newton_pair || j < nlocal) {
                  //f_conc[j][k] += wd * (conc[i][k]-conc[j][k])/rho[i]/rho[j]*(Df[itype][jtype][k]*rho[i]+Df[jtype][itype][k]*rho[j]) + wd * (conc[i][k]-conc[j][k])/rho[i] * (v[i][0] * delx + v[i][1] * dely + v[i][2] * delz);
                  f_conc[j][k] += - wd * cij_c/rho[j] * (v[j][0] * delx + v[j][1] * dely + v[j][2] * delz) + wd * cij_d/rho[i]/rho[j]*(Df[itype][jtype][k]*rho[i]+Df[jtype][itype][k]*rho[j]);
                }
              }
            }
...
void PairSDPDCDiffusion::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairSDPDCDiffusion::settings(int narg, char **arg)
{
  ...
  if (grph == -1) error->all(FLERR,"Could not find group ID in SDPDCDiffusion");
  groupbit = group->bitmask[grph];
  cut_global = force->numeric(FLERR,arg[5]);
  seed = force->inumeric(FLERR,arg[6]);
  grph = group->find(arg[7]);
  if (grph == -1) error->all(FLERR,"Could not find group_rho ID in SDPDCDiffusion");
  ...
  if(narg<13){
    ...
    if (grph == -1) error->all(FLERR,"Could not find group_fines ID in SDPDCDiffusion");
    groupbit_fines = group->bitmask[grph];
  }
...
void PairSDPDCDiffusion::coeff(int narg, char **arg)
{
...
void PairSDPDCDiffusion::init_style()
{
  if (comm->ghost_velocity == 0)
    error->all(FLERR,"Pair SDPDCDiffusion requires ghost atoms store velocity");
...
double PairSDPDCDiffusion::init_one(int i, int j)
{
...
void PairSDPDCDiffusion::write_restart(FILE *fp)
{
...
void PairSDPDCDiffusion::read_restart(FILE *fp)
{
...
void PairSDPDCDiffusion::write_restart_settings(FILE *fp)
{
...
void PairSDPDCDiffusion::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDCDiffusion::single(int i, int j, int itype, int jtype, double rsq,
...
double PairSDPDCDiffusion::single_vec(int i, int j, int itype, int jtype, double rsq,
...
void PairSDPDCDiffusion::set_weight()
{
...
//   nw_max = static_cast<int> (rr*dr_inv) + 2;
//   if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDCDiffusion::set_weight.");
//   memory->create(weight1,n+1,n+1,nw_max,"pair:weight1");
//   memory->create(weight2,n+1,n+1,nw_max,"pair:weight2");
...
//     for (j = i; j <= n; j++){
//       k = static_cast<int> (cut[i][j]*dr_inv) + 1;
//       if (k > nw_max) error->all(FLERR,"Error in PairSDPDCDiffusion::set_weight - k > nw_max");
...
void PairSDPDCDiffusion::generate_wrr()
{
  int i;
  double ww[3][3];
...
int PairSDPDCDiffusion::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
  //double *solid_rho = atom->solid_rho;
...
void PairSDPDCDiffusion::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  //double *solid_rho = atom->solid_rho;
...
int PairSDPDCDiffusion::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  //double *solid_rho = atom->solid_rho;
...
void PairSDPDCDiffusion::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
  //double *solid_rho = atom->solid_rho;
