...
FixNVESDPDC::FixNVESDPDC(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  int grph;
  if (!atom->conc_flag) error->all(FLERR,"compound concentration is not defined, unable to use fix/nve/sdpdc");
  if (narg != 5+atom->individual) error->all(FLERR,"Illegal fix nve/sdpdc command");
  if (!(strcmp(arg[3],"x")==0 || 
        strcmp(arg[3],"y")==0 || 
        strcmp(arg[3],"z")==0 || 
        strcmp(arg[3],"xy")==0 || 
...
/* ---------------------------------------------------------------------- */
FixNVESDPDC::~FixNVESDPDC(){
  memory->destroy(C0);
  delete [] resetbd;
}

/* ----------------------------------------------------------------------- */
int FixNVESDPDC::setmask()
{
  int mask = 0;
...
void FixNVESDPDC::initial_integrate(int vflag)
{
  double dtfm, dtfmom;
...
void FixNVESDPDC::final_integrate()
{
  double dtfm, dtfmom;
...
void FixNVESDPDC::initial_integrate_respa(int vflag, int ilevel, int iloop)
{
...
void FixNVESDPDC::final_integrate_respa(int ilevel, int iloop)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
