...
ComputeGroupGroupRegion::ComputeGroupGroupRegion(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  group2(NULL)
{
  if (narg < 4) error->all(FLERR,"Illegal compute group/group/region command");
  ...
  if (jgroup == -1)
    error->all(FLERR,"Compute group/group/region group ID does not exist");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"pair") == 0) {
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal compute group/group/region command");
      if (strcmp(arg[iarg+1],"yes") == 0) pairflag = 1;
      else if (strcmp(arg[iarg+1],"no") == 0) pairflag = 0;
      else error->all(FLERR,"Illegal compute group/group/region command");
      iarg += 2;
      ...
      if (molflag != OFF && atom->molecule_flag == 0)
        error->all(FLERR,"Compute group/group/region molecule requires molecule IDs");
      iarg += 2;
      ...
      if (iregion == -1)
        error->all(FLERR,"Region ID for group/group/region does not exist");
...
ComputeGroupGroupRegion::~ComputeGroupGroupRegion()
{
  delete [] group2;
  delete [] vector;
  delete [] idregion;
...
void ComputeGroupGroupRegion::init()
{
  //check validity of the region ID
  if (iregion >= 0) {
    iregion = domain->find_region(idregion);
    if (iregion == -1)
      error->all(FLERR,"Region ID for compute group/group/region does not exist");
  }
  ...
  if (pairflag && force->pair == NULL)
    error->all(FLERR,"No pair style defined for compute group/group/region");
  if (force->pair_match("hybrid",0) == NULL && force->pair->single_enable == 0)
    error->all(FLERR,"Pair style does not support compute group/group/region");

  // error if Kspace style does not compute group/group/region interactions

  if (kspaceflag && force->kspace == NULL)
    error->all(FLERR,"No Kspace style defined for compute group/group/region");
  if (kspaceflag && force->kspace->group_group_enable == 0)
    error->all(FLERR,"Kspace style does not support compute group/group/region");
  ...
  if (kspaceflag) {
    ...
    if (fabs(e_correction) > SMALL && comm->me == 0) {
      char str[128];
      sprintf(str,"Both groups in compute group/group/region have a net charge; "
...
void ComputeGroupGroupRegion::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
double ComputeGroupGroupRegion::compute_scalar()
{
...
void ComputeGroupGroupRegion::compute_vector()
{
  invoked_scalar = invoked_vector = update->ntimestep;
...
void ComputeGroupGroupRegion::pair_contribution()
{
...
void ComputeGroupGroupRegion::kspace_contribution()
{
...
void ComputeGroupGroupRegion::kspace_correction()
{
