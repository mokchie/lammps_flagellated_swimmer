...
FixStyle(remove/string,FixRemoveString)

#else

#ifndef LMP_FIX_REMOVE_STRING_H
#define LMP_FIX_REMOVE_STRING_H

#include "fix.h"

namespace LAMMPS_NS {

class FixRemoveString : public Fix {
 public:
  FixRemoveString(class LAMMPS *, int, char **);
  ~FixRemoveString();
  int setmask();
  void init();
  void post_integrate();
