...
ComputeStyle(angle/local/phase,ComputeAngleLocalPhase)

#else

#ifndef LMP_COMPUTE_ANGLE_LOCAL_PHASE_H
#define LMP_COMPUTE_ANGLE_LOCAL_PHASE_H

#include "compute.h"

namespace LAMMPS_NS {

class ComputeAngleLocalPhase : public Compute {
 public:
  ComputeAngleLocalPhase(class LAMMPS *, int, char **);
  ~ComputeAngleLocalPhase();
  void init();
  void compute_local();
  double memory_usage();
