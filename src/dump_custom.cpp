  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = &atom->conc[0][0];
        nstride = atom->individual;
      } else if (thresh_array[ithresh] == CONC2) {
        if (!atom->conc_flag || atom->individual<2){
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        }
        ptr = &atom->conc[0][1];
        nstride = atom->individual; 
      } else if (thresh_array[ithresh] == CONC3) {
        if (!atom->conc_flag || atom->individual<3){
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        }
        ptr = &atom->conc[0][2];
        nstride = atom->individual;                
      } else if (thresh_array[ithresh] == CONC4) {
        if (!atom->conc_flag || atom->individual<4){
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        }
        ptr = &atom->conc[0][3];
        nstride = atom->individual; 
      } else if (thresh_array[ithresh] == CONC5) {
        if (!atom->conc_flag || atom->individual<5){
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        }
        ptr = &atom->conc[0][4];
        nstride = atom->individual;   
      } else if (thresh_array[ithresh] == CONC6) {
        if (!atom->conc_flag || atom->individual<6){
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        }
        ptr = &atom->conc[0][5];
        ...
        else if (thresh_array[ithresh] == CTENSOR11) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor[0][0][0];
        nstride = 9;
      }
        else if (thresh_array[ithresh] == CTENSOR22) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor[0][1][1];
        nstride = 9;
      }
        else if (thresh_array[ithresh] == CTENSOR33) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor[0][2][2];
        nstride = 9;
      }
        else if (thresh_array[ithresh] == CTENSOR12) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor[0][0][1];
        nstride = 9;
      }
        else if (thresh_array[ithresh] == CTENSOR13) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor[0][0][2];
        nstride = 9;
      }
        else if (thresh_array[ithresh] == KAPPA11) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][0][0];
        nstride = 9;

      }
        else if (thresh_array[ithresh] == KAPPA12) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][0][1];
        nstride = 9;

      }
      else if (thresh_array[ithresh] == KAPPA13) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][0][2];
        nstride = 9;

      }
      else if (thresh_array[ithresh] == KAPPA21) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][1][0];
        nstride = 9;

      }
      else if (thresh_array[ithresh] == KAPPA22) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][1][1];
        nstride = 9;

      }
      else if (thresh_array[ithresh] == KAPPA23) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][1][2];
        nstride = 9;

      }
      else if (thresh_array[ithresh] == KAPPA31) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][2][0];
        nstride = 9;

      }
      else if (thresh_array[ithresh] == KAPPA32) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][2][1];
        nstride = 9;

      }
      else if (thresh_array[ithresh] == KAPPA33) {
        if (!atom->ctensor_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->kappa[0][2][2];
        nstride = 9;

      }
        else if (thresh_array[ithresh] == CEIGEN1) {
        if (!atom->ctensor_flag || !atom->eigen_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor_eigenval[0][0];
        nstride = 3;
      }
        else if (thresh_array[ithresh] == CEIGEN2) {
        if (!atom->ctensor_flag || !atom->eigen_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor_eigenval[0][1];
        nstride = 3;
      }
        else if (thresh_array[ithresh] == CEIGEN3) {
        if (!atom->ctensor_flag || !atom->eigen_flag)
          error->all(FLERR,
                     "Threshold for an atom property that isn't allocated");
        ptr = &atom->ctensor_eigenval[0][2];
        nstride = 3;
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"conc2") == 0) {
      pack_choice[i] = &DumpCustom::pack_conc2;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"conc3") == 0) {
      pack_choice[i] = &DumpCustom::pack_conc3;
      vtype[i] = Dump::DOUBLE;      
    } else if (strcmp(arg[iarg],"conc4") == 0) {
      pack_choice[i] = &DumpCustom::pack_conc4;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"conc5") == 0) {
      pack_choice[i] = &DumpCustom::pack_conc5;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"conc6") == 0) {
      pack_choice[i] = &DumpCustom::pack_conc6;
      vtype[i] = Dump::DOUBLE;
      ...
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ctensor11") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ctensor11;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ctensor22") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ctensor22;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ctensor33") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ctensor33;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ctensor12") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ctensor12;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ctensor13") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ctensor13;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ctensor23") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ctensor23;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa11") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa11;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa12") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa12;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa13") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa13;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa21") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa21;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa22") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa22;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa23") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa23;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa31") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa31;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa32") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa32;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"kappa33") == 0) {
      if (!atom->ctensor_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_kappa33;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ceigen1") == 0) {
      if (!atom->ctensor_flag || !atom->eigen_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ceigen1;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ceigen2") == 0) {
      if (!atom->ctensor_flag || !atom->eigen_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ceigen2;
      vtype[i] = Dump::DOUBLE;
    } else if (strcmp(arg[iarg],"ceigen3") == 0) {
      if (!atom->ctensor_flag || !atom->eigen_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_ceigen3;
      vtype[i] = Dump::DOUBLE;      
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"conc4") == 0) thresh_array[nthresh] = CONC4;
    else if (strcmp(arg[1],"conc5") == 0) thresh_array[nthresh] = CONC5;
    else if (strcmp(arg[1],"conc6") == 0) thresh_array[nthresh] = CONC6;
    ...
    else if (strcmp(arg[1],"ctensor11") == 0) thresh_array[nthresh] = CTENSOR11;
    else if (strcmp(arg[1],"ctensor22") == 0) thresh_array[nthresh] = CTENSOR22;  
    else if (strcmp(arg[1],"ctensor33") == 0) thresh_array[nthresh] = CTENSOR33;
    else if (strcmp(arg[1],"ctensor12") == 0) thresh_array[nthresh] = CTENSOR12;
    else if (strcmp(arg[1],"ctensor13") == 0) thresh_array[nthresh] = CTENSOR13;  
    else if (strcmp(arg[1],"ctensor23") == 0) thresh_array[nthresh] = CTENSOR23;
    else if (strcmp(arg[1],"kappa11") == 0) thresh_array[nthresh] = KAPPA11;
    else if (strcmp(arg[1],"kappa12") == 0) thresh_array[nthresh] = KAPPA12;  
    else if (strcmp(arg[1],"kappa13") == 0) thresh_array[nthresh] = KAPPA13;
    else if (strcmp(arg[1],"kappa21") == 0) thresh_array[nthresh] = KAPPA21;
    else if (strcmp(arg[1],"kappa22") == 0) thresh_array[nthresh] = KAPPA22;  
    else if (strcmp(arg[1],"kappa23") == 0) thresh_array[nthresh] = KAPPA23;
    else if (strcmp(arg[1],"kappa31") == 0) thresh_array[nthresh] = KAPPA31;
    else if (strcmp(arg[1],"kappa32") == 0) thresh_array[nthresh] = KAPPA32;
    else if (strcmp(arg[1],"kappa33") == 0) thresh_array[nthresh] = KAPPA33;   
    else if (strcmp(arg[1],"ceigen1") == 0) thresh_array[nthresh] = CEIGEN1;
    else if (strcmp(arg[1],"ceigen2") == 0) thresh_array[nthresh] = CEIGEN2;  
    else if (strcmp(arg[1],"ceigen3") == 0) thresh_array[nthresh] = CEIGEN3;    
...
void DumpCustom::pack_conc1(int n)
{
  double **conc = atom->conc;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = conc[clist[i]][0];
    n += size_one;
  }
}
...
void DumpCustom::pack_conc2(int n)
{
  double **conc = atom->conc;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = conc[clist[i]][1];
    n += size_one;
  }
}
...
void DumpCustom::pack_conc3(int n)
{
  double **conc = atom->conc;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = conc[clist[i]][2];
    n += size_one;
  }
}
...
void DumpCustom::pack_conc4(int n)
{
  double **conc = atom->conc;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = conc[clist[i]][3];
    n += size_one;
  }
}
...
void DumpCustom::pack_conc5(int n)
{
  double **conc = atom->conc;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = conc[clist[i]][4];
    n += size_one;
  }
}
...
void DumpCustom::pack_conc6(int n)
{
  double **conc = atom->conc;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = conc[clist[i]][5];
    n += size_one;
  }
}
...
void DumpCustom::pack_ctensor11(int n)
{
  double ***ctensor = atom->ctensor;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor[clist[i]][0][0];
    n += size_one;
  }
}
/* ---------------------------------------------------------------------- */

void DumpCustom::pack_ctensor22(int n)
{
  double ***ctensor = atom->ctensor;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor[clist[i]][1][1];
    n += size_one;
  }
}
/* ---------------------------------------------------------------------- */

void DumpCustom::pack_ctensor33(int n)
{
  double ***ctensor = atom->ctensor;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor[clist[i]][2][2];
    n += size_one;
  }
}
...
void DumpCustom::pack_ctensor12(int n)
{
  double ***ctensor = atom->ctensor;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor[clist[i]][0][1];
    n += size_one;
  }
} 
...
void DumpCustom::pack_ctensor13(int n)
{
  double ***ctensor = atom->ctensor;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor[clist[i]][0][2];
    n += size_one;
  }
}  
...
void DumpCustom::pack_ctensor23(int n)
{
  double ***ctensor = atom->ctensor;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor[clist[i]][1][2];
    n += size_one;
  }
}  
...
void DumpCustom::pack_kappa11(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][0][0];
    n += size_one;
  }
}  
...
void DumpCustom::pack_kappa12(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][0][1];
    n += size_one;
  }
}  
...
void DumpCustom::pack_kappa13(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][0][2];
    n += size_one;
  }
}  
...
void DumpCustom::pack_kappa21(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][1][0];
    n += size_one;
  }
} 
...
void DumpCustom::pack_kappa22(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][1][1];
    n += size_one;
  }
} 
...
void DumpCustom::pack_kappa23(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][1][2];
    n += size_one;
  }
} 
...
void DumpCustom::pack_kappa31(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][2][0];
    n += size_one;
  }
} 
...
void DumpCustom::pack_kappa32(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][2][1];
    n += size_one;
  }
} 
...
void DumpCustom::pack_kappa33(int n)
{
  double ***kappa = atom->kappa;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = kappa[clist[i]][2][2];
    n += size_one;
  }
} 
/* ---------------------------------------------------------------------- */

void DumpCustom::pack_ceigen1(int n)
{
  double **ctensor_eigenval = atom->ctensor_eigenval;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor_eigenval[clist[i]][0];
    n += size_one;
  }
}
...
void DumpCustom::pack_ceigen2(int n)
{
  double **ctensor_eigenval = atom->ctensor_eigenval;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor_eigenval[clist[i]][1];
    n += size_one;
  }
}
...
void DumpCustom::pack_ceigen3(int n)
{
  double **ctensor_eigenval = atom->ctensor_eigenval;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = ctensor_eigenval[clist[i]][2];
    n += size_one;
  }
}
