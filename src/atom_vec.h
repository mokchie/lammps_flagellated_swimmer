...
class AtomVec : protected Pointers {
 public:
  ...
  int moment_type;                     // 1 if per-type sdpd with angular momentum
  int individual;                      // 1 individual bonds, angles, dihedrals
  ...
  int pack_bond(double **);
  void write_bond(FILE *, int, double **, int);
  int pack_angle(double **);
  void write_angle(FILE *, int, double **, int);
  int pack_dihedral(double **);
  void write_dihedral(FILE *, int, double **, int);
  ...
  virtual imageint return_image(int, int *);
