...
Output::Output(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  max_stat = 0;
  n_stress = 0;
  every_stat_calc = NULL;
  every_stat_write = NULL;
  start_calc = NULL;
  next_stat_calc = NULL;
  next_stat_write = NULL;
  last_stat_calc = NULL;
  last_stat_write = NULL;
  stat = NULL;
  stress_id = NULL;
...
Output::~Output()
{
  ...
  memory->destroy(every_stat_calc);
  memory->destroy(every_stat_write);
  memory->destroy(start_calc);
  memory->destroy(next_stat_calc);
  memory->destroy(next_stat_write);
  memory->destroy(last_stat_calc);
  memory->destroy(last_stat_write);
  for (int i = 0; i < nstat; i++) delete stat[i];
  memory->sfree(stat);
  if (n_stress) memory->sfree(stress_id);
...
void Output::init()
{
  ...
  for (i = 0; i < nstat; i++){ 
    stat[i]->init();
    if (strcmp(stat[i]->style,"stress") == 0) n_stress++;   
  }
  if (n_stress){
    stress_id = (int *) memory->smalloc(n_stress*sizeof(int),"output:stress_id"); 
    j = 0;
    for (i = 0; i < nstat; i++)
      if (strcmp(stat[i]->style,"stress") == 0){
        stress_id[j] = i;
        j++;
      }
  }

  for (i = 0; i < ndump; i++) dump[i]->init();
  for (i = 0; i < ndump; i++)
    if (every_dump[i] == 0) {
...
void Output::setup(int memflag)
{
  ...
  //first calculation of statistic can be at t = 0

  if(nstat && update->restrict_output == 0){
    for (int istat = 0; istat<nstat; istat++) {
      if(every_stat_write[istat] && every_stat_calc[istat]) {
        if (next_stat_calc[istat] < ntimestep){
          if (ntimestep <= start_calc[istat])
            next_stat_calc[istat] = start_calc[istat] + 1;
          else
            next_stat_calc[istat] = ntimestep; 
        }
        if (next_stat_write[istat] < ntimestep){
          if (ntimestep <= start_calc[istat])
            next_stat_write[istat] = start_calc[istat] + static_cast<bigint>(every_stat_write[istat]);
          else
            next_stat_write[istat] = start_calc[istat] + static_cast<bigint>((ntimestep-start_calc[istat])/every_stat_write[istat] + 1)*every_stat_write[istat];
        }
      }
      else
        error->all(FLERR,"Stat every write and calc variable returned a bad timestep");
      if (next_stat_calc[istat] == ntimestep) {
        if(last_stat_calc[istat] != ntimestep) {
          stat[istat]->calc_stat();
          last_stat_calc[istat] = ntimestep;
        }
        next_stat_calc[istat] += every_stat_calc[istat];
      }
      if (istat) {
        next_calc = MIN(next_calc,next_stat_calc[istat]);
        next_stat_write_any = MIN(next_stat_write_any,next_stat_write[istat]);
      }
      else {
        next_calc = next_stat_calc[0];
        next_stat_write_any = next_stat_write[0];
      }
    }
  } 
  else next_stat_write_any = update->laststep + 1;
  ...
  next = MIN(next,next_stat_write_any);
}

/* ---------------------------------------------------------------------- */

void Output::calc(bigint ntimestep)
{
  for (int istat = 0; istat<nstat; istat++) {
    if (next_stat_calc[istat] == ntimestep) {
      if(last_stat_calc[istat] != ntimestep) {
...
{
  if(next_stat_write_any == ntimestep) { 
    for (int istat = 0; istat<nstat; istat++) {
      if(next_stat_write[istat] == ntimestep){
        if(last_stat_write[istat] != ntimestep) {
          stat[istat] -> write_stat(ntimestep);
          last_stat_write[istat] = ntimestep;
        }
        next_stat_write[istat] += every_stat_write[istat];
      }
      if (istat) {
        next_stat_write_any = MIN(next_stat_write_any,next_stat_write[istat]);
      }
...
/* ----------------------------------------------------------------------
   add a statistic to list of statistics
------------------------------------------------------------------------- */

void Output::add_statistic(int narg, char **arg)
{
  if (narg != 10 && narg != 16) error->all(FLERR,"Illegal statistic command");
  ...
  if (nstat == max_stat) {
    max_stat += DELTA;
    stat = (Statistic **)
      memory->srealloc(stat,max_stat*sizeof(Statistic *),"output:stat");
    memory->grow(every_stat_write,max_stat,"output:every_stat_write");
    memory->grow(every_stat_calc,max_stat,"output:every_stat_calc");
    memory->grow(start_calc,max_stat,"output:start_calc");
    memory->grow(next_stat_calc,max_stat,"output:next_stat_calc");
    memory->grow(next_stat_write,max_stat,"output:next_stat_write");
    memory->grow(last_stat_calc,max_stat,"output:last_stat_calc");
    memory->grow(last_stat_write,max_stat,"output:last_stat_write");
  }
...
#define STATISTIC_CLASS
#define StatisticStyle(key,Class) \
  else if (strcmp(arg[0],#key) == 0) stat[nstat] = new Class(lmp,narg,arg);
#include "style_statistic.h"
#undef STATISTIC_CLASS

  else error->all(FLERR,"Unknown statistic style");
 
  start_calc[nstat] = static_cast<bigint>(stat[nstat]->st_start); 
  next_stat_calc[nstat] = start_calc[nstat] + 1;
  every_stat_calc[nstat] = stat[nstat]->step_each;
  every_stat_write[nstat] = stat[nstat]->dump_each;
  next_stat_write[nstat] = start_calc[nstat] + static_cast<bigint>(every_stat_write[nstat]);
  last_stat_calc[nstat] = -1;
  last_stat_write[nstat] = -1;
  nstat++;
}
...
void Output::memory_usage()
{
  ...
  //for (int i = 0; i < nstat; i++) bytes += stat[i]->memory_usage();
