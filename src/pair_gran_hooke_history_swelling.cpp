...
PairGranHookeHistorySwelling::PairGranHookeHistorySwelling(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 1;
...
PairGranHookeHistorySwelling::~PairGranHookeHistorySwelling()
{
  if (copymode) return;
...
void PairGranHookeHistorySwelling::compute(int eflag, int vflag)
{
  ...
  if (ct<tstart) sr = 1.0;
  else if (ct>tend) sr = swelling_ratio;
  else sr = (ct-tstart)/(tend-tstart)*(swelling_ratio-1.0)+1.0;
...
void PairGranHookeHistorySwelling::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairGranHookeHistorySwelling::settings(int narg, char **arg)
{
  ...
  tstart = force->numeric(FLERR,arg[6]);
  tend = force->numeric(FLERR,arg[7]);
  swelling_ratio  = force->numeric(FLERR,arg[8]);

  if (dampflag == 0) gammat = 0.0;

  if (kn < 0.0 || kt < 0.0 || gamman < 0.0 || gammat < 0.0 ||
      xmu < 0.0 || xmu > 10000.0 || dampflag < 0 || dampflag > 1 || 
      tstart < 0.0 || tend < 0.0 || tend < tstart || swelling_ratio < 1.0)
...
void PairGranHookeHistorySwelling::coeff(int narg, char **arg)
{
...
void PairGranHookeHistorySwelling::init_style()
{
  int i;
...
double PairGranHookeHistorySwelling::init_one(int i, int j)
{
  if (!allocated) allocate();
...
void PairGranHookeHistorySwelling::write_restart(FILE *fp)
{
...
void PairGranHookeHistorySwelling::read_restart(FILE *fp)
{
...
void PairGranHookeHistorySwelling::write_restart_settings(FILE *fp)
{
  ...
  fwrite(&tstart,sizeof(double),1,fp);
  fwrite(&tend,sizeof(double),1,fp);  
  fwrite(&swelling_ratio,sizeof(double),1,fp);
}
...
void PairGranHookeHistorySwelling::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
    ...
    fread(&swelling_ratio,sizeof(double),1,fp);
  }
  ...
  MPI_Bcast(&tstart,1,MPI_DOUBLE,0,world);
  MPI_Bcast(&tend,1,MPI_DOUBLE,0,world);
  MPI_Bcast(&swelling_ratio,1,MPI_DOUBLE,0,world);
}

/* ---------------------------------------------------------------------- */

void PairGranHookeHistorySwelling::reset_dt()
{
  dt = update->dt;
}
...
double PairGranHookeHistorySwelling::single(int i, int j, int /*itype*/, int /*jtype*/,
                                    double rsq,
...
int PairGranHookeHistorySwelling::pack_forward_comm(int n, int *list, double *buf,
...
void PairGranHookeHistorySwelling::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
double PairGranHookeHistorySwelling::memory_usage()
{
