...
/* ----------------------------------------------------------------------
   compute the average distance to the center of mass of a group of atoms
------------------------------------------------------------------------- */
void Group::r2cm(int igroup, int plane, double *ra, double *cc){
  int n1,n2;
  if (plane==XY) {n1 = 0; n2 = 1;}
  else if (plane==XZ) {n1 = 0; n2 = 2;}
  else if (plane==YZ) {n1 = 1; n2 = 2;}
  ...
  int nlocal = atom->nlocal;
  int gcount = count(igroup);
  double cone[3];
  cone[0] = cone[1] = cone[2] = 0.0;
  double rr=0.0, rr_all;
  double unwrap[3];
  ...
  MPI_Allreduce(cone,cc,3,MPI_DOUBLE,MPI_SUM,world);
  cc[0] /= gcount;
  cc[1] /= gcount;
  cc[2] /= gcount;
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      domain->unmap(x[i],image[i],unwrap);
      rr += sqrt((unwrap[n1]-cc[n1])*(unwrap[n1]-cc[n1]) + (unwrap[n2]-cc[n2])*(unwrap[n2]-cc[n2]));
    }
  MPI_Allreduce(&rr,&rr_all,1,MPI_DOUBLE,MPI_SUM,world);
  rr_all /= gcount;
  *ra = rr_all;
}
...
/* ----------------------------------------------------------------------
   compute the simple linear regression in xy plane for a group of atoms
------------------------------------------------------------------------- */

void Group::lr(int igroup, int plane, double *slope, double *intercept){
  int n1,n2;
  if (plane==XY) {n1 = 0; n2 = 1;}
  else if (plane==XZ) {n1 = 0; n2 = 2;}
  else if (plane==YZ) {n1 = 1; n2 = 2;}
  ...
  double cone[3];
  cone[0] = cone[1] = cone[2] = 0.0;
  double xmym=0.0,xmm=0.0,xmym_all,xmm_all;
  double unwrap[3];
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      domain->unmap(x[i],image[i],unwrap);
      xmym += (unwrap[n1]-cc[n1])*(unwrap[n2]-cc[n2]);
      xmm += (unwrap[n1]-cc[n1])*(unwrap[n1]-cc[n1]);
    }
    MPI_Allreduce(&xmym,&xmym_all,1,MPI_DOUBLE,MPI_SUM,world);
    MPI_Allreduce(&xmm,&xmm_all,1,MPI_DOUBLE,MPI_SUM,world);
    if(xmm_all<EPSILON) error->all(FLERR,"slope is infinite");
    else{
      *slope = xmym_all/xmm_all;
      *intercept = cc[n2] - *slope*cc[n1];
    }
