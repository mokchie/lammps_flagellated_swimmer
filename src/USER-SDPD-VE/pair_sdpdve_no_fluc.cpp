...
/* ---------------------------------------------------------------------- */
/* DSYEV prototype */
/* DSYEV determines the eigenvalues and eigenvectors of a real symmetric matrix */
extern "C"{
...
PairSDPDVENoFluc::PairSDPDVENoFluc(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
  ...
  comm_forward = 16; 
  comm_reverse = 16; // These two numbers both should be 1+9+6=16 because besides rho we also need to sum up the vel gradient tensor
}

/* ---------------------------------------------------------------------- */

PairSDPDVENoFluc::~PairSDPDVENoFluc()
{
  if (allocated) {
...
void PairSDPDVENoFluc::compute(int eflag, int vflag)
{
  int i,j,k,l,ll,ii,jj,kk,inum,jnum,itype,jtype;
  double sum;
  double PI_i[3][3], PI_j[3][3], fcon[3], randn[3];
  ...
  double eigenval[3]={0.0};
  double eigenvec[9]={0.0};
  double iA[9] = {0.0};
  double iB[9] = {0.0};
  int ipiv[3];
  int ldb = 3;
  int ilwork = 192; // ilwork is the length of the work space for the dsysv solver, 192 is its optimal value
  double *iwork;
  iwork = (double*)malloc( ilwork*sizeof(double) );
  int info,lwork = 102; // lwork is the length of the work space for the dsyev solver, 102 is its optimal value
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            kappa[i][1][2] += wdf*dely*delvz;
            kappa[i][2][0] += wdf*delz*delvx;
            kappa[i][2][1] += wdf*delz*delvy;
            kappa[i][2][2] += wdf*delz*delvz;

            chi[i][0][0] += wdf*delx*delx;
            chi[i][0][1] += wdf*delx*dely;
            chi[i][0][2] += wdf*delx*delz;
            //chi[i][1][0] += wdf*dely*delx;
            chi[i][1][1] += wdf*dely*dely;
            chi[i][1][2] += wdf*dely*delz;
            //chi[i][2][0] += wdf*delz*delx;
            //chi[i][2][1] += wdf*delz*dely;
            chi[i][2][2] += wdf*delz*delz;
            if (newton_pair || j < nlocal){
              rho[j] += wd; 
              kappa[j][0][0] += wdf*delx*delvx;
              kappa[j][0][1] += wdf*delx*delvy;
              kappa[j][0][2] += wdf*delx*delvz;
              kappa[j][1][0] += wdf*dely*delvx;
              kappa[j][1][1] += wdf*dely*delvy;
              kappa[j][1][2] += wdf*dely*delvz;
              kappa[j][2][0] += wdf*delz*delvx;
              kappa[j][2][1] += wdf*delz*delvy;
              kappa[j][2][2] += wdf*delz*delvz;
              chi[j][0][0] += wdf*delx*delx;
              chi[j][0][1] += wdf*delx*dely;
              chi[j][0][2] += wdf*delx*delz;
              //chi[j][1][0] += wdf*dely*delx;
              chi[j][1][1] += wdf*dely*dely;
              chi[j][1][2] += wdf*dely*delz;
              //chi[j][2][0] += wdf*delz*delx;
              //chi[j][2][1] += wdf*delz*dely;
              chi[j][2][2] += wdf*delz*delz;
            }
          }
        }
      }
  ...
  // communicate and sum densities & velocity gradient tensor
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            r = sqrt(rsq);
            if (r < EPSILON) continue;     // r can be 0.0 in SDPDVENoEdynNoEdyn systems
            rinv = 1.0/r;
            delvx = vxtmp - v[j][0];
            delvy = vytmp - v[j][1];
            delvz = vztmp - v[j][2];
            ...
            for (k=0; k<3; k++){
                fcon[k] = ((PI_i[k][0]/rho[i]/rho[i] + PI_j[k][0]/rho[j]/rho[j]) * delx + (PI_i[k][1]/rho[i]/rho[i] + PI_j[k][1]/rho[j]/rho[j]) * dely + (PI_i[k][2]/rho[i]/rho[i] + PI_j[k][2]/rho[j]/rho[j]) * delz) * wd * factor_dpd; 
            }
            ...
            fnc[1] = fcon[1] + factor_dpd*(sig*wrr[1] - delvy*gam - 0.5*gam*(delz*omx - delx*omz));
	          fnc[2] = fcon[2] + factor_dpd*(sig*wrr[2] - delvz*gam - 0.5*gam*(delx*omy - dely*omx));
  ...
  for(i=0; i<nlocal; i++){
    ...
    //     fcr[j][k] = 0.0;
    //     for(l=0; l<3; l++){
    //       fcr[j][k] += tNpinvsqrt*sqrt(eigenval[l])*randn[l]*dtinvsqrt*eigenvec[l*3+j]*eigenvec[l*3+k];
    //     }
    //     if(k>j){
    //       fcr[k][j] = fcr[j][k];
    //     }
...
void PairSDPDVENoFluc::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairSDPDVENoFluc::settings(int narg, char **arg)
{
  ...
  */
  ...
  if (narg>=12){
    grph = group->find(arg[11]);
    if (grph == -1) error->all(FLERR,"Could not find group_cons ID in SDPD");
    groupbit_cons = group->bitmask[grph];
  } else groupbit_cons = 0;
  if (narg>=13){
    kappa_order = force->inumeric(FLERR,arg[12]);
    if (kappa_order<1 || kappa_order>2) error->all(FLERR,"Illegal pair_style command: kappa_order = 1 or 2");
  }
  else{
    kappa_order = 2;
  }
...
void PairSDPDVENoFluc::coeff(int narg, char **arg)
{
...
void PairSDPDVENoFluc::init_style()
{
...
double PairSDPDVENoFluc::init_one(int i, int j)
{
...
void PairSDPDVENoFluc::write_restart(FILE *fp)
{
...
void PairSDPDVENoFluc::read_restart(FILE *fp)
{
...
void PairSDPDVENoFluc::write_restart_settings(FILE *fp)
{
  ...
  fwrite(&kappa_order,sizeof(int),1,fp);
}
...
void PairSDPDVENoFluc::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
  ...
  if (comm->me == 0) {
    ...
    fread(&kappa_order,sizeof(int),1,fp);
  }
  ...
  MPI_Bcast(&kappa_order,1,MPI_INT,0,world);
...
double PairSDPDVENoFluc::single(int i, int j, int itype, int jtype, double rsq,
...
void PairSDPDVENoFluc::set_weight()
{
  ...
  if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDVENoFluc::set_weight.");
  ...
  for (i = 1; i <= n; i++)
    for (j = i; j <= n; j++){
      ...
      if (k > nw_max) error->all(FLERR,"Error in PairSDPDVENoFluc::set_weight - k > nw_max");
...
void PairSDPDVENoFluc::generate_wrr()
{
  int i;
  double ww[3][3];
...
int PairSDPDVENoFluc::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPDVENoFluc::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
int PairSDPDVENoFluc::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
void PairSDPDVENoFluc::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
void PairSDPDVENoFluc::print_matrix( char* desc, int m, int n, double** a) {
  int i, j;
  printf( "\n %s\n", desc );
  for( i = 0; i < m; i++ ) {
