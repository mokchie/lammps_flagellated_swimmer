...
PairSDPD::PairSDPD(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
void PairSDPD::write_restart(FILE *fp)
{
...
void PairSDPD::read_restart(FILE *fp)
{
...
void PairSDPD::write_restart_settings(FILE *fp)
{
...
void PairSDPD::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPD::single(int i, int j, int itype, int jtype, double rsq,
...
double PairSDPD::single_vec(int i, int j, int itype, int jtype, double rsq,
...
void PairSDPD::set_weight()
{
  ...
  nw_max = static_cast<int> (rr*dr_inv) + 2;
  if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPD::set_weight.");
  memory->create(weight1,n+1,n+1,nw_max,"pair:weight1");
  memory->create(weight2,n+1,n+1,nw_max,"pair:weight2");
...
int PairSDPD::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPD::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
int PairSDPD::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
void PairSDPD::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
