...
#define EPS 1.0e-3 // EPS is the criteria we use to decide whether two eigenvals are equal
#define EPSR 1.0e-4 // EPSR is the criteria we use to decide whether an eigenvector must be renormalized
#define DR 0.01

/* ---------------------------------------------------------------------- */

PairSDPDVE::PairSDPDVE(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
void PairSDPDVE::compute(int eflag, int vflag)
{
  int i,j,k,l,ll,ii,jj,kk,inum,jnum,itype,jtype;
  double sum;
  double PI_i[3][3], PI_j[3][3],fcon[3],kappa_eigen[3][3],H[3][3],randn[3];
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            r = sqrt(rsq);
            if (r < EPSILON) continue;     // r can be 0.0 in SDPDVE systems
            rinv = 1.0/r;
            delvx = vxtmp - v[j][0];
            delvy = vytmp - v[j][1];
            delvz = vztmp - v[j][2];
            ...
            for (k=0; k<3; k++){
              fcon[k] = ((PI_i[k][0]/rho[i]/rho[i] + PI_j[k][0]/rho[j]/rho[j]) * delx + (PI_i[k][1]/rho[i]/rho[i] + PI_j[k][1]/rho[j]/rho[j]) * dely + (PI_i[k][2]/rho[i]/rho[i] + PI_j[k][2]/rho[j]/rho[j]) * delz) * wd; 
            }
            ...
            fnc[1] = factor_dpd*fcon[1] + factor_dpd*(sig*wrr[1] - delvy*gam - 0.5*gam*(delz*omx - delx*omz));
	          fnc[2] = factor_dpd*fcon[2] + factor_dpd*(sig*wrr[2] - delvz*gam - 0.5*gam*(delx*omy - dely*omx));
...
void PairSDPDVE::settings(int narg, char **arg)
{
...
void PairSDPDVE::coeff(int narg, char **arg)
{
...
double PairSDPDVE::init_one(int i, int j)
{
...
void PairSDPDVE::write_restart(FILE *fp)
{
...
void PairSDPDVE::read_restart(FILE *fp)
{
...
void PairSDPDVE::write_restart_settings(FILE *fp)
{
...
void PairSDPDVE::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDVE::single(int i, int j, int itype, int jtype, double rsq,
...
void PairSDPDVE::set_weight()
{
  ...
  if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDVE::set_weight.");
  ...
  for (i = 1; i <= n; i++)
    for (j = i; j <= n; j++){
      ...
      if (k > nw_max) error->all(FLERR,"Error in PairSDPDVE::set_weight - k > nw_max");
...
int PairSDPDVE::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
void PairSDPDVE::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
int PairSDPDVE::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
...
void PairSDPDVE::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
  double ***kappa = atom->kappa;
