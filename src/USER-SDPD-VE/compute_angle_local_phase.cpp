...
#include <cmath>
#include <cstring>
#include "compute_angle_local_phase.h"
#include "atom.h"
#include "atom_vec.h"
#include "molecule.h"
#include "update.h"
...
ComputeAngleLocalPhase::ComputeAngleLocalPhase(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  bstyle(NULL), vvar(NULL), tstr(NULL), vstr(NULL), vlocal(NULL), alocal(NULL)
{
  if (narg < 4) error->all(FLERR,"Illegal compute angle/local/phase command");

  if (atom->avec->angles_allow == 0)
    error->all(FLERR,"Compute angle/local/phase used when angles are not allowed");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"set") == 0) {
      setflag = 1;
      if (iarg+3 > narg) error->all(FLERR,"Illegal compute angle/local/phase command");
	...
	tflag = 1;
      } else error->all(FLERR,"Illegal compute angle/local/phase command");
      iarg += 3;
    } else error->all(FLERR,"Illegal compute angle/local/phase command");
  }
  ...
  if (nvar) {
    if (!setflag)
      error->all(FLERR,"Compute angle/local/phase variable requires a set variable");
    for (int i = 0; i < nvar; i++) {
      vvar[i] = input->variable->find(vstr[i]);
      if (vvar[i] < 0)
	error->all(FLERR,"Variable name for copute angle/local/phase does not exist");
      if (!input->variable->equalstyle(vvar[i]))
	error->all(FLERR,"Variable for compute angle/local/phase is invalid style");
    }

    if (tstr) {
      tvar = input->variable->find(tstr);
      if (tvar < 0)
        error->all(FLERR,"Variable name for compute angle/local/phase does not exist");
      if (!input->variable->internalstyle(tvar))
        error->all(FLERR,"Variable for compute angle/local/phase is invalid style");
    }
  } else if (setflag) 
    error->all(FLERR,"Compute angle/local/phase set with no variable");
...
ComputeAngleLocalPhase::~ComputeAngleLocalPhase()
{
  delete [] bstyle;
...
void ComputeAngleLocalPhase::init()
{
  if (force->angle == NULL)
    error->all(FLERR,"No angle style is defined for compute angle/local/phase");
...
void ComputeAngleLocalPhase::compute_local()
{
...
int ComputeAngleLocalPhase::compute_angles(int flag)
{
...
void ComputeAngleLocalPhase::reallocate(int n)
{
  ...
  if (nvalues == 1) {
    memory->destroy(vlocal);
    memory->create(vlocal,nmax,"angle/local/phase:vector_local");
    vector_local = vlocal;
  } else {
    memory->destroy(alocal);
    memory->create(alocal,nmax,nvalues,"angle/local/phase:array_local");
    array_local = alocal;
  }
}
...
double ComputeAngleLocalPhase::memory_usage()
{
