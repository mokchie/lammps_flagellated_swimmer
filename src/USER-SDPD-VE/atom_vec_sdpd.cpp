...
AtomVecSDPD::AtomVecSDPD(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecSDPD::copy(int i, int j, int delflag)
{
  int k;
...
int AtomVecSDPD::pack_comm(int n, int *list, double *buf,
...
int AtomVecSDPD::pack_comm_vel(int n, int *list, double *buf,
                                    int pbc_flag, int *pbc)
{
  int i,j,m;
...
void AtomVecSDPD::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPD::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPD::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPD::pack_reverse_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPD::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPD::unpack_reverse_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPD::pack_border(int n, int *list, double *buf,
                                  int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecSDPD::pack_border_vel(int n, int *list, double *buf,
                                      int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecSDPD::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecSDPD::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPD::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPD::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPD::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecSDPD::unpack_exchange(double *buf)
{
  int k;
...
int AtomVecSDPD::size_restart()
{
  ...
  if (individual){
    for (i = 0; i < nlocal; i++)
      n += 20 + 3*num_bond[i] + na*num_angle[i] +
        6*num_dihedral[i] + 5*num_improper[i];
  }
  else{
    for (i = 0; i < nlocal; i++)
      n += 20 + 2*num_bond[i] + 4*num_angle[i] +
...
int AtomVecSDPD::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecSDPD::unpack_restart(double *buf)
{
  int k;
...
void AtomVecSDPD::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecSDPD::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecSDPD::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecSDPD::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecSDPD::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecSDPD::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecSDPD::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecSDPD::memory_usage()
{
  bigint bytes = 0;
