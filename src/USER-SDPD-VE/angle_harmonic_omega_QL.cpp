...
AngleHarmonicOmegaQL::AngleHarmonicOmegaQL(LAMMPS *lmp) : Angle(lmp)
{
  random = NULL;
  mtrand = NULL;
  k = NULL;
...
AngleHarmonicOmegaQL::~AngleHarmonicOmegaQL()
{
  if (allocated) {
...
void AngleHarmonicOmegaQL::compute(int eflag, int vflag)
{
  ...
  for (i=1; i<nat+1; i++){
    ...
    if (T_t[i]<=0){
      ...
      if (first_learn[i]){
        ...
        //printf("rnd=%f\n",rnd);
        reward[i] = (dist0[i]-dist[i])/(2*MY_PI/fabs(omega[i]));
...
void AngleHarmonicOmegaQL::allocate()
{
  allocated = 1;  
  int seed;
...
void AngleHarmonicOmegaQL::coeff(int narg, char **arg)
{  
...
double AngleHarmonicOmegaQL::equilibrium_angle(int i)
{
  return theta0[i];
}
...
void AngleHarmonicOmegaQL::write_restart(FILE *fp)
{
...
void AngleHarmonicOmegaQL::read_restart(FILE *fp)
{
  allocate();
...
void AngleHarmonicOmegaQL::write_data(FILE *fp)
{
...
/* ---------------------------------------------------------------------- */
double AngleHarmonicOmegaQL::query_omega(int atype){
  return omega[atype];
}
double AngleHarmonicOmegaQL::single(int type, int i1, int i2, int i3)
{
  double **x = atom->x;
...
// void AngleHarmonicOmegaQL::update_omega(){
//   int n = atom->nangletypes;
//   double rg;
//   for (int i=1; i<=n; i++){
//     T_t[i]-=update->dt;
...
// }
double AngleHarmonicOmegaQL::shiftmap(double x, double a0, double b0, double a, double b)
{
