...
AtomVecSDPDVE::AtomVecSDPDVE(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecSDPDVE::grow(int n)
{
  ...
  /*added code starts*/
  kappa = memory->grow(atom->kappa,nmax,3,3,"atom:kappa");
  chi = memory->grow(atom->chi,nmax,3,3,"atom:chi");
  ctensor = memory->grow(atom->ctensor,nmax,3,3,"atom:ctensor");
  ctensor_eigenval = memory->grow(atom->ctensor_eigenval,nmax,3,"atom:ctensor_eigenval");
  ctensor_eigenvec = memory->grow(atom->ctensor_eigenvec,nmax,3,3,"atom:ctensor_eigenvec");
  f_ctensor_eigenval = memory->grow(atom->f_ctensor_eigenval,nmax*comm->nthreads,3,"atom:f_ctensor_eigenval");
  f_ctensor_eigenvec = memory->grow(atom->f_ctensor_eigenvec,nmax*comm->nthreads,3,3,"atom:f_ctensor_eigenvec");
  /*added code ends*/
  ...
  if (individual){
    bond_length = memory->grow(atom->bond_length,nmax,atom->bond_per_atom,"atom:bond_length");
    angle_area = memory->grow(atom->angle_area,nmax,atom->angle_per_atom,"atom:angle_area");
    dihedral_angle = memory->grow(atom->dihedral_angle,nmax,atom->dihedral_per_atom,"atom:dihedral_angle");
  }
...
void AtomVecSDPDVE::grow_reset()
{
  ...
  kappa = atom->kappa;
  chi = atom->chi;
  ctensor = atom->ctensor;
  ctensor_eigenval = atom->ctensor_eigenval;
  ctensor_eigenvec = atom->ctensor_eigenvec;
  f_ctensor_eigenval = atom->f_ctensor_eigenval;
  f_ctensor_eigenvec = atom->f_ctensor_eigenvec;
  /*added code ends*/
  rho = atom->rho;
  spin = atom->spin;
  ...
  if (individual){
    ...
    angle_area = atom->angle_area;
    dihedral_angle = atom->dihedral_angle;
  }
}
...
void AtomVecSDPDVE::copy(int i, int j, int delflag)
{
  int k,l;
  ...
  for (k=0; k<3; k++)
  {
    ctensor_eigenval[j][k] = ctensor_eigenval[i][k];
    for (l=0; l<3; l++)
    {
      ctensor_eigenvec[j][k][l] = ctensor_eigenvec[i][k][l];
      ctensor[j][k][l] = ctensor[i][k][l];
    }
  }
  /*added code ends*/
  ...
  if (individual){
    for (k = 0; k < num_bond[j]; k++)
      bond_length[j][k] = bond_length[i][k];
    for (k = 0; k < num_angle[j]; k++)
      angle_area[j][k] = angle_area[i][k];
    for (k = 0; k < num_dihedral[j]; k++)
      dihedral_angle[j][k] = dihedral_angle[i][k]; 
  }
...
int AtomVecSDPDVE::pack_comm(int n, int *list, double *buf,
...
int AtomVecSDPDVE::pack_comm_vel(int n, int *list, double *buf,
                                    int pbc_flag, int *pbc)
{
  int i,j,m,k,l; // added variables k amd l
  ...
  if (pbc_flag == 0) {
    for (i = 0; i < n; i++) {
      ...
      /* added code starts */
      for (k=0; k<3; k++){
        buf[m++] = ctensor_eigenval[j][k];
      }
      for (k=0; k<3; k++) {
        for (l=0; l<3; l++){
          buf[m++] = ctensor_eigenvec[j][k][l];
        }
      }
      /* added code ends */
    }
...
void AtomVecSDPDVE::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDVE::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last,k,l;
  ...
  for (i = first; i < last; i++) {
    ...
    /* added code starts */
    for (k=0; k<3; k++){
      ctensor_eigenval[i][k] = buf[m++];
    }
    for (k=0; k<3; k++) {
      for (l=0; l<3; l++){
        ctensor_eigenvec[i][k][l] = buf[m++];
      }
    }
    /* added code ends */
  }
...
int AtomVecSDPDVE::pack_reverse(int n, int first, double *buf)
{
...
int AtomVecSDPDVE::pack_reverse_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDVE::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPDVE::unpack_reverse_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPDVE::pack_border(int n, int *list, double *buf,
                                  int pbc_flag, int *pbc)
{
  //store an atom's info in a buffer communicated on neighbor re−builds
  int i,j,m,k,l;
  double dx,dy,dz;
  ...
  if (pbc_flag == 0) {
    ...
    for (i = 0; i < n; i++) {
      ...
      buf[m++] = ubuf(mask[j]).d;
      buf[m++] = ubuf(molecule[j]).d;
      buf[m++] = ubuf(return_image(j,pbc)).d;
    }
  }
...
int AtomVecSDPDVE::pack_border_vel(int n, int *list, double *buf,
                                      int pbc_flag, int *pbc)
{
  int i,j,m,k,l;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecSDPDVE::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecSDPDVE::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDVE::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last,k,l;
...
int AtomVecSDPDVE::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPDVE::pack_exchange(int i, double *buf)
{
  ...
  /* added code starts */
  for (k=0; k<3; k++){
    buf[m++] = ctensor_eigenval[i][k];
  }
  for (k=0; k<3; k++) {
    for (l=0; l<3; l++){
      buf[m++] = ctensor_eigenvec[i][k][l];
    }
  }
  /* added code ends */
  ...
  if (individual){
    for (k = 0; k < num_bond[i]; k++)
      buf[m++] = bond_length[i][k];
    for (k = 0; k < num_angle[i]; k++)
      buf[m++] = angle_area[i][k];
    for (k = 0; k < num_dihedral[i]; k++)
...
int AtomVecSDPDVE::unpack_exchange(double *buf)
{
  int k,l;
  ...
  /* added code starts */
  for (k=0; k<3; k++){
    ctensor_eigenval[nlocal][k] = buf[m++];
  }
  for (k=0; k<3; k++) {
    for (l=0; l<3; l++){
      ctensor_eigenvec[nlocal][k][l] = buf[m++];
    }
  }
  /* added code ends */
  ...
  if (individual){
    for (k = 0; k < num_bond[nlocal]; k++)
      bond_length[nlocal][k] = buf[m++];
    for (k = 0; k < num_angle[nlocal]; k++)
      angle_area[nlocal][k] = buf[m++];
    for (k = 0; k < num_dihedral[nlocal]; k++)
      dihedral_angle[nlocal][k] = buf[m++];
  }
...
int AtomVecSDPDVE::size_restart()
{
  int i;
  ...
  if (individual){
    for (i = 0; i < nlocal; i++)
      n += 32 + 3*num_bond[i] + 5*num_angle[i] +
        6*num_dihedral[i] + 5*num_improper[i];
  }
  else{
    for (i = 0; i < nlocal; i++)
      n += 32 + 2*num_bond[i] + 4*num_angle[i] +
...
int AtomVecSDPDVE::pack_restart(int i, double *buf)
{
  int k,l;
...
int AtomVecSDPDVE::unpack_restart(double *buf)
{
  int k,l;
...
void AtomVecSDPDVE::create_atom(int itype, double *coord)
{
  int k,l;
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
  ...
  /* added code starts */
  for (k=0; k<3; k++){
    ctensor_eigenval[nlocal][k] = 0.0;
  }
  for (k=0; k<3; k++) {
    for (l=0; l<3; l++){
      ctensor_eigenvec[nlocal][k][l] = 0.0;
      ctensor[nlocal][k][l] = 0.0;
      kappa[nlocal][k][l] = 0.0;
      chi[nlocal][k][l] = 0.0;
    }
  }
  /* added code ends */
...
void AtomVecSDPDVE::data_atom(double *coord, imageint imagetmp, char **values)
{
  int k,l;
  ...
  //here we randomly initialize the conformation tensor
  for (k=0; k<2; k++){
     ctensor_eigenval[nlocal][k] = 2.0*(double)rand()/RAND_MAX;
  }
  if (domain->dimension == 3) 
    ctensor_eigenval[nlocal][2] = 2.0*(double)rand()/RAND_MAX;
  else ctensor_eigenval[nlocal][2] = 1.0;

  alpha = (double) rand()/RAND_MAX*2.0*M_PI;
  if (domain->dimension == 3)
    beta = (double) rand()/RAND_MAX*2.0*M_PI;
  else beta = 0.0;
  gamma = (double) rand()/RAND_MAX*2.0*M_PI;
  
  ctensor_eigenvec[nlocal][0][0] = cos(alpha)*cos(gamma) - cos(beta)*sin(alpha)*sin(gamma);
  ctensor_eigenvec[nlocal][0][1] = sin(alpha)*cos(gamma) + cos(beta)*cos(alpha)*sin(gamma);
  ctensor_eigenvec[nlocal][0][2] = sin(beta)*sin(gamma);
  ctensor_eigenvec[nlocal][1][0] = -cos(alpha)*sin(gamma)-cos(beta)*sin(alpha)*cos(gamma);
  ctensor_eigenvec[nlocal][1][1] = -sin(alpha)*sin(gamma)+cos(beta)*cos(alpha)*cos(gamma);
  ctensor_eigenvec[nlocal][1][2] = sin(beta)*cos(gamma);
  ctensor_eigenvec[nlocal][2][0] = sin(beta)*sin(alpha);
  ctensor_eigenvec[nlocal][2][1] = -sin(beta)*cos(alpha);
  ctensor_eigenvec[nlocal][2][2] = cos(beta);
  /* added code ends */
...
int AtomVecSDPDVE::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecSDPDVE::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecSDPDVE::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecSDPDVE::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecSDPDVE::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecSDPDVE::memory_usage()
{
  bigint bytes = 0;
  ...
  /* added code starts */
  if (atom->memcheck("kappa")) bytes += memory->usage(kappa,nmax,3,3);
  if (atom->memcheck("chi")) bytes += memory->usage(chi,nmax,3,3);
  if (atom->memcheck("ctensor")) bytes += memory->usage(ctensor,nmax,3,3);
  if (atom->memcheck("ctensor_eigenval")) bytes += memory->usage(ctensor_eigenval,nmax,3);
  if (atom->memcheck("ctensor_eigenvec")) bytes += memory->usage(ctensor_eigenvec,nmax,3,3);
  if (atom->memcheck("f_ctensor_eigenval")) bytes += memory->usage(f_ctensor_eigenval,nmax*comm->nthreads,3);
  if (atom->memcheck("f_ctensor_eigenvec")) bytes += memory->usage(f_ctensor_eigenvec,nmax*comm->nthreads,3,3);
  /* added code ends */
  ...
  if (individual){
    if (atom->memcheck("bond_length"))
      bytes += memory->usage(bond_length,nmax,atom->bond_per_atom);
    if (atom->memcheck("angle_area"))
      bytes += memory->usage(angle_area,nmax,atom->angle_per_atom);
    if (atom->memcheck("dihedral_angle"))
      bytes += memory->usage(dihedral_angle,nmax,atom->dihedral_per_atom);
  }
  return bytes;
}
