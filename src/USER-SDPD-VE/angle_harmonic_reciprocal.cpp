...
AngleHarmonicReciprocal::AngleHarmonicReciprocal(LAMMPS *lmp) : Angle(lmp)
{
  k = NULL;
  b = NULL;
  omega = NULL;
...
AngleHarmonicReciprocal::~AngleHarmonicReciprocal()
{
  if (allocated) {
...
void AngleHarmonicReciprocal::compute(int eflag, int vflag)
{
  int i1,i2,i3,n,type,nc,m=-1,i;
  double bi;
  double delx1,dely1,delz1,delx2,dely2,delz2;
  double eangle,f1[3],f3[3];
  double dtheta,theta,thetaeq,tk,thx,cross,phi;
  ...
  for (n = 0; n < nanglelist; n++) {
    ...
    if (cross > 0.0)
      theta = -theta;
    phi = omega[type]*update->ntimestep*update->dt;
    ...
    if (tau[type]>=EPS)
      thetaeq = theta0[type] + (1.0-exp(-update->ntimestep*update->dt/tau[type])) * bi*sin(phi)*sin(thx);
    else
      thetaeq = theta0[type] + bi*sin(phi)*sin(thx);
...
void AngleHarmonicReciprocal::allocate()
{
  allocated = 1;
...
void AngleHarmonicReciprocal::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 7) error->all(FLERR,"Incorrect args for angle coefficients");
  ...
  double tau_one = 0.0;
  if (narg >= 6)
    skew_one = force->numeric(FLERR,arg[5]);
  if (narg >= 7)
    tau_one = force->numeric(FLERR,arg[6]);
...
double AngleHarmonicReciprocal::equilibrium_angle(int i)
{
  return theta0[i];
}
...
void AngleHarmonicReciprocal::write_restart(FILE *fp)
{
...
void AngleHarmonicReciprocal::read_restart(FILE *fp)
{
  allocate();
...
void AngleHarmonicReciprocal::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nangletypes; i++)
    fprintf(fp,"%d %g %g %g %g %g %g\n",i,k[i],b[i]/MY_PI*180.0,omega[i],theta0[i]/MY_PI*180.0,skew[i]/MY_PI*180.0,tau[i]);
}

/* ---------------------------------------------------------------------- */

double AngleHarmonicReciprocal::single(int type, int i1, int i2, int i3)
{
  double **x = atom->x;
...
double AngleHarmonicReciprocal::shiftmap(double x, double a0, double b0, double a, double b)
{
