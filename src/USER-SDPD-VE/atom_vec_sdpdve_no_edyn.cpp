...
AtomVecSDPDVENoEdyn::AtomVecSDPDVENoEdyn(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecSDPDVENoEdyn::grow(int n)
{
  if (n == 0) grow_nmax();
  else nmax = n;
  atom->nmax = nmax;
  ...
  f_ctensor = memory->grow(atom->f_ctensor,nmax,3,3,"atom:f_ctensor");
  /*added code ends*/
...
void AtomVecSDPDVENoEdyn::grow_reset()
{
...
void AtomVecSDPDVENoEdyn::copy(int i, int j, int delflag)
{
  int k,l;
...
int AtomVecSDPDVENoEdyn::pack_comm(int n, int *list, double *buf,
...
int AtomVecSDPDVENoEdyn::pack_comm_vel(int n, int *list, double *buf,
                                    int pbc_flag, int *pbc)
{
...
void AtomVecSDPDVENoEdyn::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDVENoEdyn::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last,k,l;
...
int AtomVecSDPDVENoEdyn::pack_reverse(int n, int first, double *buf)
{
...
int AtomVecSDPDVENoEdyn::pack_reverse_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecSDPDVENoEdyn::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPDVENoEdyn::unpack_reverse_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecSDPDVENoEdyn::pack_border(int n, int *list, double *buf,
                                  int pbc_flag, int *pbc)
{
...
int AtomVecSDPDVENoEdyn::pack_border_vel(int n, int *list, double *buf,
                                      int pbc_flag, int *pbc)
{
  int i,j,m,k,l;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecSDPDVENoEdyn::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
  m = 0;
  for (i = 0; i < n; i++) {
...
void AtomVecSDPDVENoEdyn::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
  m = 0;
  last = first + n;
...
void AtomVecSDPDVENoEdyn::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last,k,l;
...
int AtomVecSDPDVENoEdyn::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecSDPDVENoEdyn::pack_exchange(int i, double *buf)
{
...
int AtomVecSDPDVENoEdyn::unpack_exchange(double *buf)
{
  int k,l;
...
int AtomVecSDPDVENoEdyn::size_restart()
{
  int i;
  ...
  int n = 0;
  int na = 5;
  if (abs(individual) > 1) na = 6;  
  if (individual){
    for (i = 0; i < nlocal; i++)
      n += 29 + 3*num_bond[i] + na*num_angle[i] +
        6*num_dihedral[i] + 5*num_improper[i];
  }
  else{
    for (i = 0; i < nlocal; i++)
      n += 29 + 2*num_bond[i] + 4*num_angle[i] +
...
int AtomVecSDPDVENoEdyn::pack_restart(int i, double *buf)
{
  int k,l;
...
int AtomVecSDPDVENoEdyn::unpack_restart(double *buf)
{
  int k,l;
...
void AtomVecSDPDVENoEdyn::create_atom(int itype, double *coord)
{
  int k,l;
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
  ...
  for (k=0; k<3; k++) {
    for (l=0; l<3; l++){
      if(k==l) ctensor[nlocal][k][l] = 1.0;
      else ctensor[nlocal][k][l] = 0.0;
...
void AtomVecSDPDVENoEdyn::data_atom(double *coord, imageint imagetmp, char **values)
{
  int k,l;
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecSDPDVENoEdyn::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecSDPDVENoEdyn::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecSDPDVENoEdyn::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecSDPDVENoEdyn::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecSDPDVENoEdyn::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecSDPDVENoEdyn::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("f_ctensor")) bytes += memory->usage(f_ctensor,nmax,3,3);
  /* added code ends */
