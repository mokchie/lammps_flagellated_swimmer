...
BondHybrid::~BondHybrid()
{
  ...
  if (allocated) {
    ...
    for (int i = 0; i < nstyles; i++) {
      ...
      memory->destroy(bondlist_length[i]);
    }
    delete [] bondlist;
    delete [] bondlist_length;
  }
}
...
void BondHybrid::compute(int eflag, int vflag)
{
  ...
  double *bondlist_length_orig = neighbor->bondlist_length;
  ...
  if (neighbor->ago == 0) {
    ...
    for (m = 0; m < nstyles; m++) {
      if (nbondlist[m] > maxbond[m]) {
        ...
        memory->destroy(bondlist_length[m]);
        maxbond[m] = nbondlist[m] + EXTRA;
        memory->create(bondlist[m],maxbond[m],3,"bond_hybrid:bondlist");
        memory->create(bondlist_length[m],maxbond[m],"bond_hybrid:bondlist_length");
      }
      nbondlist[m] = 0;
    }
    for (i = 0; i < nbondlist_orig; i++) {
      ...
      bondlist_length[m][n] = bondlist_length_orig[i];
      nbondlist[m]++;
    }
  }
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    neighbor->bondlist_length = bondlist_length[m];
  ...
  neighbor->bondlist_length = bondlist_length_orig;
}
...
void BondHybrid::allocate()
{
  ...
  bondlist_length = new double*[nstyles];
  for (int m = 0; m < nstyles; m++) maxbond[m] = 0;
  for (int m = 0; m < nstyles; m++) {
    bondlist[m] = NULL;
    bondlist_length[m] = NULL;
  }
}
