...
FixSquirmer::FixSquirmer(LAMMPS *lmp, int narg, char **arg) :
  ...
  avec_line(NULL), avec_tri(NULL), fcm_active(NULL), torque_active(NULL), orientation(NULL)
{
  ...
  memory->create(fcm_active,nbody,3,"rigid:fcm_active");
  ...
  memory->create(torque_active,nbody,3,"rigid:torque_active");
  memory->create(orientation,nbody,3,"rigid:orientation");  
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"force") == 0) {
      ...
      if (iarg+2 > narg)
        error->all(FLERR,"Illegal fix squirmer npt/nph command");
      if (strcmp(arg[iarg+1],"all") == 0) allremap = 1;
        ...
        if (idilate == -1)
          error->all(FLERR,
                     "Fix squirmer npt/nph dilate group ID does not exist");
      }
      iarg += 2;

    } else if (strcmp(arg[iarg],"dpd") == 0) {
        if (iarg+2 > narg)
          error->all(FLERR,"Illegal fix squirmer npt/nph command");
        delete [] id_dpd;
        int n = strlen(arg[iarg+1]) + 1;
        id_dpd = new char[n];
        strcpy(id_dpd,arg[iarg+1]);
        idpd = group->find(id_dpd);
        if (idpd == -1)
          error->all(FLERR,
      ...
      else if (strcmp(arg[iarg+1],"x") == 0) inistyle = EX;
      else if (strcmp(arg[iarg+1],"y") == 0) inistyle = EY;
      else if (strcmp(arg[iarg+1],"z") == 0) inistyle = EZ;
      else if (strcmp(arg[iarg+1],"-x") == 0) inistyle = NEX;
      else if (strcmp(arg[iarg+1],"-y") == 0) inistyle = NEY;
      else if (strcmp(arg[iarg+1],"-z") == 0) inistyle = NEZ;      
      else {inistyle = RND; randflag=1;}

      if (!randflag && iarg+2 > narg) error->all(FLERR,"Illegal fix rigid command");
      if (randflag && iarg+3 > narg) error->all(FLERR,"Illegal fix rigid command");
      if (randflag){ 
        seed_ori = force->inumeric(FLERR,arg[iarg+2]);
        if (seed_ori < 0)
          error->all(FLERR,"Illegal fix rigid command");
      }
      if(randflag) iarg += 3;
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rigid command");
      Rs = force->numeric(FLERR,arg[iarg+1]);
      iarg += 2;

    } else if (strcmp(arg[iarg],"B1") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rigid command");
      B1 = force->numeric(FLERR,arg[iarg+1]);
      iarg += 2;
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"fixflag") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix rigid command");
      fixflag = force->inumeric(FLERR,arg[iarg+1]);
      iarg += 2;
  ...
  if (randflag) random_ori = new RanMars(lmp,seed_ori + me);
  else random_ori = NULL;
  ...
  for (i = 0; i < nbody; i++) {
    ...
    fcm_active[i][0] = fcm_active[i][1] = fcm_active[i][2] = 0.0;
...
void FixSquirmer::setup_pre_neighbor()
{
  pre_neighbor();
}
...
void FixSquirmer::setup(int vflag)
{
  int i,n,ibody;
...
void FixSquirmer::apply_langevin_thermostat()
{
  if (me == 0) {
...
void FixSquirmer::enforce2d()
{
  for (int ibody = 0; ibody < nbody; ibody++) {
    xcm[ibody][2] = 0.0;
    vcm[ibody][2] = 0.0;
    fcm[ibody][2] = 0.0;
    fcm_active[ibody][2] = 0.0;
    torque[ibody][0] = 0.0;
    torque[ibody][1] = 0.0;
    torque_active[ibody][0] = 0.0;
    torque_active[ibody][1] = 0.0;
    angmom[ibody][0] = 0.0;
    angmom[ibody][1] = 0.0;
    omega[ibody][0] = 0.0;
    omega[ibody][1] = 0.0;
...
void FixSquirmer::compute_forces_and_torques()
{
  int i,ibody;
...
/* ---------------------------------------------------------------------- */
void FixSquirmer::squirmer_slip_velocity(double *rs, double *ex, double B1, double beta, double *v){
  double costheta = ex[0]*rs[0] + ex[1]*rs[1] + ex[2]*rs[2];
  //printf("costheta=%f\n",costheta);
  v[0] = B1 * (1 + beta*costheta) * (costheta * rs[0] - ex[0]);
  v[1] = B1 * (1 + beta*costheta) * (costheta * rs[1] - ex[1]);
  v[2] = B1 * (1 + beta*costheta) * (costheta * rs[2] - ex[2]);
  //printf("%f,%f,%f\n",v[0],v[1],v[2]);
}
...
void FixSquirmer::final_integrate()
{
  ...
  for (ibody=0; ibody<nbody; ibody++){
    fcm_active[ibody][0] = fcm_active[ibody][1] = fcm_active[ibody][2] = 0.0;
    torque_active[ibody][0] = torque_active[ibody][1] = torque_active[ibody][2] = 0.0;
  } 
  for (ibody = 0; ibody < nbody; ibody++)
    for (int i = 0; i < 6; i++) sum[ibody][i] = 0.0; 
  if (idpd>-1){
    double **x = atom->x;
    double **v = atom->v;
    double **f = atom->f;
      ...
      if ((mask[i] & dpd_group_bit) && body[i]<0) {
        ...
        for (ibody=0; ibody<nbody; ibody++){
          ...
          if ((rcm < Rs+TOLERANCE) && (rcm > Rs-TOLERANCE)){
            ...
            sum[ibody][0] -= deltap[0]/dtv;
            sum[ibody][1] -= deltap[1]/dtv;
            sum[ibody][2] -= deltap[2]/dtv;

            sum[ibody][3] -= (xhitn[1]*Rs*deltap[2] - xhitn[2]*Rs*deltap[1]) / dtv;
            sum[ibody][4] -= (xhitn[2]*Rs*deltap[0] - xhitn[0]*Rs*deltap[2]) / dtv;
            sum[ibody][5] -= (xhitn[0]*Rs*deltap[1] - xhitn[1]*Rs*deltap[0]) / dtv;
  ...
  MPI_Allreduce(sum[0],all[0],6*nbody,MPI_DOUBLE,MPI_SUM,world);
  for (ibody=0; ibody<nbody; ibody++){
    fcm_active[ibody][0] = all[ibody][0];
    fcm_active[ibody][1] = all[ibody][1];
    fcm_active[ibody][2] = all[ibody][2];
...
void FixSquirmer::initial_integrate_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixSquirmer::final_integrate_respa(int ilevel, int /*iloop*/)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
int FixSquirmer::dof(int tgroup)
{
...
void FixSquirmer::deform(int flag)
{
  if (flag == 0)
    for (int ibody = 0; ibody < nbody; ibody++)
...
void FixSquirmer::setup_bodies_static()
{
  int i,ibody;
  ...
  for (ibody = 0; ibody < nbody; ibody++) {
    ...
    if (inistyle==EX){
      ...
      MathExtra::transpose_matvec(ex_space[ibody],ey_space[ibody],
                                ez_space[ibody],eex,orientation[ibody]);
      ...
      MathExtra::transpose_matvec(ex_space[ibody],ey_space[ibody],
                                ez_space[ibody],eex,orientation[ibody]);
    } else if (inistyle==NEX){
      eex[0] = -1;
      eex[1] = 0;
      eex[2] = 0; 
      MathExtra::transpose_matvec(ex_space[ibody],ey_space[ibody],
                                ez_space[ibody],eex,orientation[ibody]);      
    } else if (inistyle==NEY){
      eex[0] = 0;
      eex[1] = -1;
      eex[2] = 0; 
      MathExtra::transpose_matvec(ex_space[ibody],ey_space[ibody],
                                ez_space[ibody],eex,orientation[ibody]);       
    } else if (inistyle==NEZ){
      eex[0] = 0;
      eex[1] = 0;
      eex[2] = -1; 
      MathExtra::transpose_matvec(ex_space[ibody],ey_space[ibody],
                                ez_space[ibody],eex,orientation[ibody]);
    } else if (inistyle==RND){
      if(domain->dimension == 3){
        phi = random_ori->uniform() * 2*MY_PI;
        theta = random_ori->uniform() * MY_PI;
        MPI_Bcast(&phi,1,MPI_DOUBLE,0,world);
        MPI_Bcast(&theta,1,MPI_DOUBLE,0,world);
        eex[0] = cos(phi)*sin(theta);
        eex[1] = sin(phi)*sin(theta);
        eex[2] = cos(theta);
        MathExtra::transpose_matvec(ex_space[ibody],ey_space[ibody],
                                ez_space[ibody],eex,orientation[ibody]);        
      } else{
...
void FixSquirmer::setup_bodies_dynamic()
{
  int i,ibody;
...
void FixSquirmer::readfile(int which, double *vec,
...
void FixSquirmer::write_restart_file(char *file)
{
  if (me) return;
...
double FixSquirmer::memory_usage()
{
  int nmax = atom->nmax;
...
void FixSquirmer::grow_arrays(int nmax)
{
...
void FixSquirmer::copy_arrays(int i, int j, int /*delflag*/)
{
  body[j] = body[i];
...
void FixSquirmer::set_arrays(int i)
{
  body[i] = -1;
  xcmimage[i] = 0;
  displace[i][0] = 0.0;
...
int FixSquirmer::pack_exchange(int i, double *buf)
{
  buf[0] = ubuf(body[i]).d;
...
int FixSquirmer::unpack_exchange(int nlocal, double *buf)
{
...
void FixSquirmer::zero_momentum()
{
  for (int ibody = 0; ibody < nbody; ibody++)
    vcm[ibody][0] = vcm[ibody][1] = vcm[ibody][2] = 0.0;
...
void FixSquirmer::zero_rotation()
{
  for (int ibody = 0; ibody < nbody; ibody++) {
    angmom[ibody][0] = angmom[ibody][1] = angmom[ibody][2] = 0.0;
...
int FixSquirmer::modify_param(int narg, char **arg)
{
...
double FixSquirmer::compute_scalar()
{
...
void *FixSquirmer::extract(const char *str, int &dim)
{
...
double FixSquirmer::extract_erotational()
{
  double wbody[3],rot[3][3];
...
double FixSquirmer::compute_array(int i, int j)
{
