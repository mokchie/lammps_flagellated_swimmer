...
PairSDPDMC::PairSDPDMC(LAMMPS *lmp) : Pair(lmp)
{
  random = NULL;
  mtrand = NULL;
...
void PairSDPDMC::compute(int eflag, int vflag)
{
  int i,j,k,l,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,evdwl,fpair,fcon;
...
if (grain==NULL) 
    error->all(FLERR,"pair_style sdpd/mc only work with atom_style sdpd/mc, where the array 'grain' is defined");    
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (mask[j] & groupbit){
          ...
          if (rsq < cutsq[itype][jtype]) {
            ...
            fcon = ((p_i/rho[i]/rho[i] + p_j/rho[j]/rho[j])*wd)*factor_dpd; // conservative force
	    ...
	    fnc[2] = factor_dpd*(sig*wrr[2] - delvz*gam - 0.5*gam*(delx*omy - dely*omx));
            ...
            if (grain[i] == 1){
              f[i][0] += delx*fpair + delx*fcon*scale[itype][jtype] + fnc[0];
              f[i][1] += dely*fpair + dely*fcon*scale[itype][jtype] + fnc[1];
              f[i][2] += delz*fpair + delz*fcon*scale[itype][jtype] + fnc[2];  
            }
            else{
              f[i][0] += delx*(fpair+fcon) + fnc[0];
    	        f[i][1] += dely*(fpair+fcon) + fnc[1];
	            f[i][2] += delz*(fpair+fcon) + fnc[2];
            }
            ...
            if (newton_pair || j < nlocal) {
              if (grain[j] == 1){
                f[j][0] -= delx*fpair + delx*fcon*scale[jtype][itype] + fnc[0];
                f[j][1] -= dely*fpair + dely*fcon*scale[jtype][itype] + fnc[1];
                f[j][2] -= delz*fpair + delz*fcon*scale[jtype][itype] + fnc[2];                
              }
              else{
                f[j][0] -= delx*(fpair+fcon) + fnc[0];
      	        f[j][1] -= dely*(fpair+fcon) + fnc[1];
	              f[j][2] -= delz*(fpair+fcon) + fnc[2];
              }
            ...
            if (n_stress){
              ind_stress = 0;
              ff[0] = delx*(delx*(fpair+fcon) + fnc[0]);
              ff[1] = dely*(dely*(fpair+fcon) + fnc[1]);
              ff[2] = delz*(delz*(fpair+fcon) + fnc[2]);
              ff[3] = delx*(dely*(fpair+fcon) + fnc[1]);
              ff[4] = delx*(delz*(fpair+fcon) + fnc[2]);
              ff[5] = dely*(delz*(fpair+fcon) + fnc[2]);
...
void PairSDPDMC::settings(int narg, char **arg)
{
...
void PairSDPDMC::coeff(int narg, char **arg)
{
  ...
  if (narg == 6) scale_one = force->numeric(FLERR,arg[5]);
...
double PairSDPDMC::init_one(int i, int j)
{
...
void PairSDPDMC::write_restart(FILE *fp)
{
...
void PairSDPDMC::read_restart(FILE *fp)
{
...
void PairSDPDMC::write_restart_settings(FILE *fp)
{
...
void PairSDPDMC::read_restart_settings(FILE *fp)
{
  long unsigned int seed2;
...
double PairSDPDMC::single(int i, int j, int itype, int jtype, double rsq,
...
double PairSDPDMC::single_vec(int i, int j, int itype, int jtype, double rsq,
...
//   if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDMCVENoEdyn2D::set_weight.");
...
//       if (k > nw_max) error->all(FLERR,"Error in PairSDPDMCVENoEdyn2D::set_weight - k > nw_max");
...
void PairSDPDMC::set_weight()
{
  ...
  nw_max = static_cast<int> (rr*dr_inv) + 2;
  if (nw_max < 1 || nw_max > 600) error->all(FLERR,"Non-positive or too large value for nw_max to initialize weight arrays: PairSDPDMC::set_weight.");
  memory->create(weight1,n+1,n+1,nw_max,"pair:weight1");
  memory->create(weight2,n+1,n+1,nw_max,"pair:weight2");
  ...
  for (i = 1; i <= n; i++)
    for (j = i; j <= n; j++){
      k = static_cast<int> (cut[i][j]*dr_inv) + 1;
      if (k > nw_max) error->all(FLERR,"Error in PairSDPDMC::set_weight - k > nw_max");
...
int PairSDPDMC::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
  double *rho = atom->rho;
...
void PairSDPDMC::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
int PairSDPDMC::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
  double *rho = atom->rho;
...
void PairSDPDMC::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
  double *rho = atom->rho;
