...
NTopo::NTopo(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  bondlist_length = anglelist_area = dihedrallist_angle = NULL;
...
NTopo::~NTopo()
{
  ...
  memory->destroy(bondlist_length);
  memory->destroy(anglelist_area);
  memory->destroy(dihedrallist_angle);
}
...
{
  maxbond = 2*atom->nbonds;
  //if (nprocs == 1) maxbond = 2*atom->nbonds;
  //else maxbond = static_cast<int> (LB_FACTOR * atom->nbonds / nprocs);
  memory->create(bondlist,maxbond,3,"neigh_topo:bondlist");
  if (atom->individual)
    memory->create(bondlist_length,maxbond,"neigh_topo:bondlist_length");
}
...
void NTopo::allocate_angle()
{
  ...
  if (atom->individual)
    memory->create(anglelist_area,maxangle,"neigh_topo:anglelist_area");
}
...
void NTopo::allocate_dihedral()
{
  ...
  if (atom->individual)
    memory->create(dihedrallist_angle,maxdihedral,"neigh_topo:dihedrallist_angle");
}
...
bigint NTopo::memory_usage()
{
  ...
  if (atom->individual){
    bytes += maxbond * sizeof(double);
    bytes += maxangle * sizeof(double);
    bytes += maxdihedral * sizeof(double); 
  }
  return bytes;
}
