...
void Neighbor::build_topology()
{
  if (force->bond) {
    neigh_bond->build();
    maxbond = neigh_bond->maxbond;
    nbondlist = neigh_bond->nbondlist;
    bondlist = neigh_bond->bondlist;
    if (atom->individual)
      bondlist_length = neigh_bond->bondlist_length;
  }
  if (force->angle) {
    neigh_angle->build();
    ...
    if (atom->individual)
      anglelist_area = neigh_angle->anglelist_area;  
  }
  if (force->dihedral) {
    ...
    if (atom->individual)
      dihedrallist_angle = neigh_dihedral->dihedrallist_angle;
  }
  if (force->improper) {
