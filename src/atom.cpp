...
Atom::Atom(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  mol_type = NULL;
  mol_size = NULL;
  mol_corresp_ind = fix_force_bound_ind = 0;
  n_mol_corresp_glob = n_mol_corresp_glob_max = 0; 
  mol_corresp_glob = NULL;
  ...
  kappa = NULL;
  chi = NULL;
  ctensor_eigenval = NULL;
  ctensor_eigenvec = NULL; 
  f_ctensor_eigenval = NULL;
  f_ctensor_eigenvec = NULL;
  f_ctensor = NULL;
  conc = NULL;
  f_conc = NULL;
  fixfine = NULL;
  ...
  rho_flag = e_flag = cv_flag = vest_flag = thermal_flag = 0;
...
Atom::~Atom()
{
  ...
  memory->destroy(mol_size);
  memory->destroy(mol_corresp_glob);
  ...
  memory->destroy(ctensor);
  memory->destroy(kappa);
  memory->destroy(chi);
  memory->destroy(ctensor_eigenval);
  memory->destroy(ctensor_eigenvec);
  memory->destroy(f_ctensor_eigenval);
  memory->destroy(f_ctensor_eigenvec);
  memory->destroy(f_ctensor);
  ...
  memory->destroy(rho);
  memory->destroy(solid_rho);  
  memory->destroy(fluid_rho);  
  memory->destroy(drho);
  memory->destroy(e);
  memory->destroy(de);
  memory->destroy(cv);
  ...
  memory->destroy(angle_area);
  memory->destroy(angle_area2);
...
void Atom::init()
{
  ...
  // calculate number of molecules and atoms per molecule
  calc_n_per_molecule();
}
...
void Atom::calc_n_per_molecule()
{
  int j, ind;
  tagint i, nnmol;
  tagint *atmol = NULL;
  tagint *atmol_recv = NULL;  
  tagint *t_tmp = NULL;
  int ntyp_max = 10;
  ...
  if (molecular){
    ...
    for (j = 0; j < nlocal; ++j)
      if (nnmol < molecule[j])
        nnmol = molecule[j];
    MPI_Allreduce(&nnmol,&n_mol_max,1,MPI_LMP_TAGINT,MPI_MAX,world);

    if (n_mol_max > 0) {
      memory->create(atmol,n_mol_max,"atom:atmol");
      memory->create(atmol_recv,n_mol_max,"atom:atmol_recv");
      ...
      MPI_Allreduce(atmol,atmol_recv,n_mol_max,MPI_LMP_TAGINT,MPI_SUM,world);
      memory->destroy(atmol);

      memory->create(t_tmp,ntyp_max,"atom:t_tmp");
      memory->create(mol_type,n_mol_max,"atom:mol_type");
      for (i = 0; i < n_mol_max; ++i){     // find different molecule types based on the molecule size (number of atoms) 
        ind = 1;
        mol_type[i] = -1;
        for (j = 0; j < n_mol_types; ++j)
          if (t_tmp[j] == atmol_recv[i]){
            mol_type[i] = j;
            ind = 0;
	    break; 
          }
        if (ind && atmol_recv[i] > 0){
          t_tmp[n_mol_types] = atmol_recv[i];
          mol_type[i] = n_mol_types;
          n_mol_types++;  // the number of different molecule types
          if (n_mol_types == ntyp_max){
            ntyp_max += 10;
            memory->grow(t_tmp,ntyp_max,"atom:t_tmp");
          }
        }
      }
      memory->destroy(atmol_recv);      

      if (n_mol_types){
        memory->create(mol_size,n_mol_types,"atom:mol_size");
        for (j = 0; j < n_mol_types; ++j)
          mol_size[j] = t_tmp[j];     // array of molecular sizes
      }
      memory->destroy(t_tmp);     
    }
  }
}
                       ...
                       int type_offset)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    *next = '\0';
    if (individual==1)
      sscanf(buf,"%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " %lg",
             &tmp,&itype,&atom1,&atom2,&atom3,&a0tmp);
    else if (individual>=2)
      sscanf(buf,"%d %d " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " %lg" " %lg",
             &tmp,&itype,&atom1,&atom2,&atom3,&a0tmp,&a1tmp);
    else
    ...
    if ((m = map(atom2)) >= 0) {
      if (count) count[m]++;
        ...
        if (individual)
          angle_area[m][num_angle[m]] = a0tmp;
        if (individual>=2)
          angle_area2[m][num_angle[m]] = a1tmp;
        num_angle[m]++;
      }
    }
    if (newton_bond == 0) {
                          ...
                          int type_offset)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    *next = '\0';
    if (individual)
      sscanf(buf,"%d %d "
             TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " " TAGINT_FORMAT " %lg",
             &tmp,&itype,&atom1,&atom2,&atom3,&atom4,&phi0tmp);
    else
      sscanf(buf,"%d %d " 
    ...
    if ((m = map(atom2)) >= 0) {
      if (count) count[m]++;
        ...
        if (individual)
          dihedral_angle[m][num_dihedral[m]] = phi0tmp;
        num_dihedral[m]++;
      }
    }
    if (newton_bond == 0) {
...
void Atom::allocate_type_arrays()
{
  ...
  if (avec->moment_type) {
    moment = new double[ntypes+1];
    moment_setflag = new int[ntypes+1];
    for (int itype = 1; itype <= ntypes; itype++) moment_setflag[itype] = 0;
  }
}
...
/* ----------------------------------------------------------------------
   set a moment and flag it as set
   called from reading of data file
   type_offset may be used when reading multiple data files
------------------------------------------------------------------------- */

void Atom::set_moment(const char *file, int line, const char *str, int type_offset)
{
  if (moment == NULL) error->all(file,line,"Cannot set moment for this atom style");

  int itype;
  double moment_one;
  int n = sscanf(str,"%d %lg",&itype,&moment_one);
  if (n != 2) error->all(file,line,"Invalid moment line in data file");
  itype += type_offset;

  if (itype < 1 || itype > ntypes) error->all(file,line,"Invalid type for moment set");

  moment[itype] = moment_one;
  moment_setflag[itype] = 1;

  if (moment[itype] <= 0.0) error->all(file,line,"Invalid moment value");
}
...
void Atom::set_moment(const char *file, int line, int itype, double value)
{
  if (moment == NULL) error->all(file,line,"Cannot set moment for this atom style");
  if (itype < 1 || itype > ntypes) 
    error->all(file,line,"Invalid type for moment set");
...
/* ----------------------------------------------------------------------
   set one or more moments and flag them as set
   called from reading of input script
------------------------------------------------------------------------- */

void Atom::set_moment(const char *file, int line, int /*narg*/, char **arg)
{
  if (moment == NULL) error->all(file,line,"Cannot set moment for this atom style");

  int lo,hi;
  force->bounds(file,line,arg[0],ntypes,lo,hi);
  if (lo < 1 || hi > ntypes) error->all(file,line,"Invalid type for moment set");

  for (int itype = lo; itype <= hi; itype++) {
    moment[itype] = atof(arg[1]);
    moment_setflag[itype] = 1;
...
/* ----------------------------------------------------------------------
   set all moments as read in from restart file
------------------------------------------------------------------------- */

void Atom::set_moment(double *values)
{
  for (int itype = 1; itype <= ntypes; itype++) {
    moment[itype] = values[itype];
    moment_setflag[itype] = 1;
  }
}

/* ----------------------------------------------------------------------
   check that all per-atom-type moments have been set
------------------------------------------------------------------------- */

void Atom::check_moment(const char *file, int line)
{
  if (moment == NULL) return;
  for (int itype = 1; itype <= ntypes; itype++)
    if (moment_setflag[itype] == 0) 
      error->all(file,line,"Not all per-type moments are set");
}
                             ...
                             int ilocal, tagint offset)
{
  ...
  if (avec->bonds_allow) {
    ...
    for (int i = 0; i < num_bond[ilocal]; i++) {
      ...
      if (individual && onemol->individual){
        bond_length[ilocal][i] = onemol->bond_length[iatom][i];
        if(bond_length0!=NULL)
          bond_length0[ilocal][i] = onemol->bond_length[iatom][i];
      }
    }
  }
  ...
  if (avec->angles_allow) {
    ...
    for (int i = 0; i < num_angle[ilocal]; i++) {
      ...
      if (individual && onemol->individual)
        angle_area[ilocal][i] = onemol->angle_area[iatom][i];
    }
  }
  ...
  if (avec->dihedrals_allow) {
    ...
    for (int i = 0; i < num_dihedral[ilocal]; i++) {
      ...
      if (individual && onemol->individual)
        dihedral_angle[ilocal][i] = onemol->dihedral_angle[iatom][i];
    }
  }
...
void *Atom::extract(char *name)
{
  ...
  if (strcmp(name,"ctensor") == 0) return (void *) ctensor;
  if (strcmp(name,"kappa") == 0) return (void *) kappa;
  if (strcmp(name,"chi") == 0) return (void *) chi;
  if (strcmp(name,"ctensor_eigenval") == 0) return (void *) ctensor_eigenval;
  if (strcmp(name,"ctensor_eigenvec") == 0) return (void *) ctensor_eigenvec;
  if (strcmp(name,"f_ctensor_eigenval") == 0) return (void *) f_ctensor_eigenval;
  if (strcmp(name,"f_ctensor_eigenvec") == 0) return (void *) f_ctensor_eigenvec; 
  if (strcmp(name,"f_ctensor") == 0) return (void *) f_ctensor;
  if (strcmp(name,"fixfine") == 0) return (void *) fixfine;   
  if (strcmp(name,"fixed") == 0) return (void *) fixed;  
  ...
  if (strcmp(name,"solid_rho") == 0) return (void *) solid_rho;  
  if (strcmp(name,"fluid_rho") == 0) return (void *) fluid_rho;  
  if (strcmp(name,"drho") == 0) return (void *) drho;
  if (strcmp(name,"e") == 0) return (void *) e;
  if (strcmp(name,"de") == 0) return (void *) de;
  if (strcmp(name,"cv") == 0) return (void *) cv;
  if (strcmp(name,"vest") == 0) return (void *) vest;
  if (strcmp(name,"pte0") == 0) return (void *) pte0;
  if (strcmp(name,"pte1") == 0) return (void *) pte1;
